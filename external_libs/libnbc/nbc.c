...
int NBC_Free(NBC_Handle* handle) {
  ...
  /* do not free schedule as it is in the cache */
...
int NBC_Progress(NBC_Handle *handle) {
   ...
   * we assume done */
  if(handle->schedule == NULL) return NBC_OK;
...
int NBC_Progress_block(NBC_Handle *handle) {
  ...
  do {
    /* the request is done or invalid if there is no schedule attached 
     * we assume done - NBC_Start_round calls NBC_Progress and this
     * might finish the request ... */
    if(handle->schedule == NULL) return NBC_OK;
