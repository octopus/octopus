...
! Define the format
 ...
 contains
  ...
  subroutine X(io_function_output_supercell) (how, dir, fname, mesh, ff, centers, supercell, unit, ierr, &
    ...
    if(i_am_root) then
      call X(io_function_output_global_supercell)(how, dir, fname, mesh, ff_global, centers, supercell, &                                               unit, ierr, namespace, geo = geo, extra_atom=extra_atom)
    end if
    if(comm /= MPI_COMM_NULL .and. comm /= 0 .and. .not. is_global_) then
      ! I have to broadcast the error code
      call MPI_Bcast(ierr, 1, MPI_INTEGER, 0, comm, mpi_err)
...
! ---------------------------------------------------------
subroutine X(io_function_output_global_supercell) (how, dir, fname, mesh, ff, centers, supercell, unit, ierr, namespace, geo, extra_atom)
  integer(8),                 intent(in)  :: how
  character(len=*),           intent(in)  :: dir, fname
  type(mesh_t),               intent(in)  :: mesh
  R_TYPE,                     intent(in)  :: ff(:,:)  !< (mesh%np_global or mesh%np_part_global)
