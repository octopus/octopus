...
! NOTE: MJV: how could this return anything but ix=0? Answ: if there is a shift in origin
  ...
  subroutine out_matlab(how, d1, d2, d3, out_what)
    ...
    iunit = io_open(filename, action='write', namespace=namespace, recl=record_length)
  ...
  subroutine out_mesh_index()
    ...
    iunit = io_open(trim(dir)//'/'//trim(fname)//".mesh_index", action='write', namespace=namespace)
  ...
  subroutine out_dx()
    ...
    iunit = io_open(trim(dir)//'/'//trim(fname)//".dx", action='write', namespace=namespace)
  ...
  subroutine out_cube()
    ...
    iunit = io_open(trim(dir)//'/'//trim(fname)//".cube", action='write', namespace=namespace)
  ...
  subroutine out_xcrysden(write_real)
    ...
    iunit = io_open(trim(dir)//'/'//trim(fname_ext)//".xsf", action='write', namespace=namespace)
