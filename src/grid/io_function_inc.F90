...
! Define the format
  ...
  if(iand(how, C_OUTPUT_HOW_VTK) /= 0) call out_vtk()
...
function X(interpolate_isolevel)(mesh, ff, isosurface_value, ip1, ip2) result(pos)
  ...
  !!    
  ! ---------------------------------------------------------
  subroutine X(out_cf_vts)(filename, ierr, cf_in, cube, unit, points)
    character(len=*),      intent(in) :: filename        !< the file name
    integer,               intent(out):: ierr            !< error message   
    type(cube_function_t), intent(in) :: cf_in           !< the cube_function to be written 
    type(cube_t),          intent(in) :: cube            !< the underlying cube mesh
    ...
    FLOAT, allocatable :: pnts(:)

    PUSH_SUB(X(out_cf_vts))
...
!     cSAFE_ALLOCATE(pnts(np*3))
!
!     nn(:)=cube%rs_n_global(:)
!     do i3 =1 , cube%rs_n_global(3)
!       do i2 =1 , cube%rs_n_global(2)
!         do i1 =1 , cube%rs_n_global(1)
!           pnts(3 * (nn(1)*nn(2)*(i3-1) + nn(3)*(i2-1) + (i1-1)) + 1) = points(i1,i2,i3,1)
!           pnts(3 * (nn(1)*nn(2)*(i3-1) + nn(3)*(i2-1) + (i1-1)) + 2) = points(i1,i2,i3,2)
!           pnts(3 * (nn(1)*nn(2)*(i3-1) + nn(3)*(i2-1) + (i1-1)) + 3) = points(i1,i2,i3,3)
!         end do
!       end do
!     end do
!
!     call io_binary_write(trim(filename), np*3, pnts(:) ,&
!                          ierr, nohead = .true., fendian = is_little_endian())
!
!     cSAFE_DEALLOCATE_A(pnts)
