...
! NOTE: MJV: how could this return anything but ix=0? Answ: if there is a shift in origin
  ...
  !-----------------------------------------------------
  ! The option used to output complex Kohn-Sham orbitals is decribed as follows:
  ! loop (ix)
  !    loop(iy)
  !       loop(iz)
...
!    type(states_t),   pointer :: st !< mark: added by Wandong Yu try to locate the complex wavefunctions
    ...
    iunit = io_open(trim(dir)//'/'//trim(fname)//".space_linear", action='write')

    write(iunit, '(2a)') 'Generated by octopus ', trim(conf%version)
    write(iunit, '(4a)') 'git: ', trim(conf%git_commit), " build: ",  trim(conf%build_time)
    write(iunit, '(i5,3f12.6)') geo%natoms, offset(1:3)

    ! Output phisical ovservables point by point
    int_unit(1:3) = 1 !<define the file according to old Gaussian format
      ...
      do iy = 1, cube%rs_n_global(2)
        do iz = 1, cube%rs_n_global(3)
           write(iunit,'(2es14.6)') units_from_atomic(unit, R_REAL(cf%X(RS)(ix, iy, iz))), &
                units_from_atomic(unit, R_AIMAG(cf%X(RS)(ix, iy, iz) ))
        end do
      end do
    end do
  ...
  end subroutine out_space_linear
...
! Define the format
  ...
  if(iand(how, OPTION__OUTPUTFORMAT__SPACE_LINEAR)  /= 0) call messages_not_implemented("Outpur_KPT with format linear xyz")
