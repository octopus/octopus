...
subroutine X(derivatives_grad)(der, ff, op_ff, ghost_update, set_bc)
  ...
  if (der%mesh%latt%nonorthogonal) then
    do ip = 1, der%mesh%np
      op_ff(ip, 1:der%dim) = matmul(der%mesh%latt%klattice_primitive(1:der%dim, 1:der%dim),op_ff(ip, 1:der%dim))
    end do
  end if
...
subroutine X(derivatives_div)(der, ff, op_ff, ghost_update, set_bc)
  ...
  if (der%mesh%latt%nonorthogonal) then
    ...
    do ip = 1, der%mesh%np_part
      ff_uvw(ip, 1:der%dim) = matmul(transpose(der%mesh%latt%klattice_primitive(1:der%dim, 1:der%dim)),ff(ip, 1:der%dim))
    end do
  else
    ff_uvw => ff
  end if
...
subroutine X(derivatives_curl)(der, ff, op_ff, ghost_update, set_bc)
  ...
  ASSERT(.not.der%mesh%latt%nonorthogonal)
    ...
    do ist = 1, uvw(1)%nst_linear
      ...
      do ip = 1, der%mesh%np
        ...
        do idim2 = 1, der%dim
          do idim1 = 1, der%dim
            tmp(idim1) = tmp(idim1) + &
              der%mesh%latt%klattice_primitive(idim1, idim2) * uvw(idim2)%X(ff_linear)(ip, ist)
          end do
        end do
        do idim1 = 1, der%dim
    ...
    do ip = 1, der%mesh%np
      do ist = 1, uvw(1)%nst_linear
        ...
        do idim2 = 1, der%dim
          do idim1 = 1, der%dim
            tmp(idim1) = tmp(idim1) + &
              der%mesh%latt%klattice_primitive(idim1, idim2) * uvw(idim2)%X(ff_pack)(ist, ip)
          end do
        end do
        do idim1 = 1, der%dim
...
contains
  subroutine uvw_to_xyz_opencl
    ...
    call accel_write_buffer(matrix_buffer, der%dim**2, der%mesh%latt%klattice_primitive)
