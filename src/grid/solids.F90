...
contains
  ...
  subroutine periodic_write_crystal(sb, geo)
    type(simul_box_t), intent(in)    :: sb
    ...
    !now calculate
    iunit = io_open('static/crystal.xyz', action='write')
    ...
    do iatom = 1, geo%natoms
      ...
      do icopy = 1, periodic_copy_num(pp)
        write(iunit, '(a, 3f12.6)') geo%atom(iatom)%label, periodic_copy_position(pp, sb, icopy)/units_out%length%factor
      end do
      call periodic_copy_end(pp)
    end do
