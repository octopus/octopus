...
contains
  ...
  integer function mesh_index(dim, nr, Lxyz_inv, ix_, periodic_dim) result(index)
    ...
    integer, optional, intent(in) :: periodic_dim      ! Number of periodic dimensions.

    integer :: i, ix(MAX_DIM)      ! ix has to go until 3, not sb%dim
    integer :: periodic_dim_
    
    periodic_dim_ = 0
    if (present(periodic_dim)) periodic_dim_ = periodic_dim
    ...
    do i = 1, dim
      if(ix(i) < nr(1, i)) then    ! first look left
        if(i <= periodic_dim_) then ! fold point
