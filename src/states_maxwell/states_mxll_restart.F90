...
contains
  ...
  !----------------------------------------------------------
  subroutine states_mxll_read_user_def(mesh, st, user_def_rs_state)
    ...
    if(parse_block('UserDefinedInitialMaxwellStates', blk) == 0) then
    ...
    do ist = 1, st%nst
       do idim = 1, zff_dim
          ...
          if (should_write) then
             ...
             call zrestart_write_mesh_function(restart, filename, gr%mesh, zff_global, err, &
                  root=root, is_global=.true.)
