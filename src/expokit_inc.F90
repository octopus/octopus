...
!! It implements the Lanczos scheme to approximate the exponential
!! of an operator. 
!! The subroutines have been modified in order to make them Fortran 90.
!! Also, some modifications have been made in order to make ZHEXPV
!! work for anti-Hermitina matrices, whereas ZGPADM has been modified
!! in order to accept imaginary time-steps.
...
!!!WARNING. Not sure what is the good one...
      break_tol = 1.0d-7
!>>>  break_tol = tol
!>>>  break_tol = anorm*tol
...
!!$                      wsp(ifree),lfree, iwsp, iexph, ns, iflag )
         call ZGPADM( ideg, mx, cmplx(sgn*t_step, CNST(0.0), PRECISION), wsp(ih),mh, &
                      wsp(ifree),lfree, iwsp, iexph, ns, iflag )
         iexph = ifree + iexph - 1
         nscale = nscale + ns
      else
...
!---  compute w = beta*V*exp(t_step*H)*e1 ...
      ...
      if ( ideg.ne.0 ) then
!---     irreducible rational Pade approximation ...
!!!WARNING!!! This becomes complex for anti-Hermitian case!!!
!!$         call ZGPADM( ideg, mx, sgn*t_step, wsp(ih),mh, &
!!$                      wsp(ifree),lfree, iwsp, iexph, ns, iflag )
         call ZGPADM( ideg, mx, cmplx(CNST(0.0), sgn*t_step, PRECISION), wsp(ih),mh, &
                      wsp(ifree),lfree, iwsp, iexph, ns, iflag )
         iexph = ifree + iexph - 1
         nscale = nscale + ns
      else
...
!!$      call ZGEMV( 'n', n,mx,hjj,wsp(iv),n,wsp(iexph),1,ZERO,w,1 )
      call ZGEMV( 'n', n,mx,hjj,wsp(iv),n,wsp(iexph),1,ZERO,w(1),1 )
!!$      beta = DZNRM2( n, w,1 )
      beta = lalg_nrm2(n, w(1:n))
      hump = MAX( hump, beta )
...
!!$----------------------------------------------------------------------|
      ...
      CMPLX :: alpha(ndeg), theta(ndeg), tmpc
