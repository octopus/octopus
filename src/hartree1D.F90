...
! I do not think this method is usable when doing 1D...
!!$#ifdef HAVE_FFTW
!!$  integer n, nnx 
!!$  real(r8) :: alpha
!!$  real(r8), pointer :: ff(:, :, :)
...
subroutine hartree_init(h, m)
  ...
  call oct_parse_int(C_string('PoissonSolver'), 1, h%solver)
  if(h%solver.ne.1) then
...
subroutine hartree_cg(h, m, pot, dist)
  ...
  real(r8) :: sa, x, r, s1, s2, s3, ak, ck, xx, yy, zz, temp
  ...
  allocate(wk(-m%nx:m%nx))

  wk = 0.0d0
  do i = 1, m%nk
    x = m%kx(i)*m%H!; x(2) = m%ky(i)*m%H; x(3) = m%kz(i)*m%H
    r = abs(x) ! sqrt(sum(x**2)) + 1e-50_r8
  ...
  do i = 1, m%np
    ...
    zk(i) = zk(i) - m%d%dlidfj(0)*wk(ix)/m%h**2
    do j = 1, m%d%norder
      zk(i) = zk(i) -  m%d%dlidfj(j)*(  &
          wk(ix+j) + wk(ix-j))/m%h**2
    end do
  end do
