  ...
  subroutine output_vector_potential(outp, namespace, space, dir, st, mesh, hm, ions)     !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MAXWELL_VECTOR_POTENTIAL)) then
      ...
      do idim = 1, space%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MAXWELL_VECTOR_POTENTIAL), dir, fname, namespace, space, &
          mesh, hm%vector_potential(1:mesh%np, idim), fn_unit, ierr, ions = ions)
      end do
    end if
