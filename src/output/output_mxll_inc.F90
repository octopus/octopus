 ...
 ! ---------------------------------------------------------
  ...
  subroutine output_states_mxll(outp, namespace, dir, st, gr, hm, geo)
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__ELECTRIC_FIELD)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__ELECTRIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MAGNETIC_FIELD)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MAGNETIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_energy_density_mxll(outp, namespace, dir, hm, gr, geo)   !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MAXWELL_ENERGY_DENSITY)) then
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MAXWELL_ENERGY_DENSITY), dir, "maxwell_energy_density", namespace, gr%mesh,&
  ...
  subroutine output_poynting_vector(outp, namespace, dir, st, gr, hm, geo)
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__POYNTING_VECTOR)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__POYNTING_VECTOR), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_transverse_rs_state(outp, namespace, dir, st, gr, geo)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__TRANS_ELECTRIC_FIELD)) then
      ...
      do idim=1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__TRANS_ELECTRIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__TRANS_MAGNETIC_FIELD)) then
      ...
      do idim=1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__TRANS_MAGNETIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_longitudinal_rs_state(outp, namespace, dir, st, gr, geo)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__LONG_ELECTRIC_FIELD)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__LONG_ELECTRIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__LONG_MAGNETIC_FIELD)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__LONG_MAGNETIC_FIELD), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
      end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_divergence_rs_state(outp, namespace, dir, st, gr, geo)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__DIV_ELECTRIC_FIELD)) then
      ...
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__DIV_ELECTRIC_FIELD), dir, "e_field_div", namespace, gr%mesh, dtmp_2, fn_unit, ierr, geo=geo)
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__DIV_MAGNETIC_FIELD)) then
      ...
      call get_divergence_field(gr, dtmp_1, dtmp_2, .false.)
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__DIV_MAGNETIC_FIELD), dir, "b_field_div", namespace, gr%mesh, dtmp_2, fn_unit, ierr, geo=geo)
      SAFE_DEALLOCATE_A(dtmp_1)
      SAFE_DEALLOCATE_A(dtmp_2)
    end if
  ...
  subroutine output_vector_potential(outp, namespace, dir, st, gr, hm, geo)     !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MAXWELL_VECTOR_POTENTIAL)) then
      ...
      do idim = 1, gr%sb%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MAXWELL_VECTOR_POTENTIAL), dir, fname, namespace, gr%mesh, hm%vector_potential(1:gr%mesh%np, idim), fn_unit, &
          ierr, geo = geo)
      end do
    end if
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__EXTERNAL_CURRENT)) then
      if (hm%current_density_ext_flag) then
        ...
        do idim = 1, st%dim
          ...
          call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__EXTERNAL_CURRENT), dir, fname, namespace, gr%mesh, dtmp(:, idim), fn_unit, ierr, geo = geo)
        end do
  ...
  subroutine output_charge_density_mxll(outp, namespace, dir, st, gr, hm, geo)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__CHARGE_DENSITY)) then
      ...
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__CHARGE_DENSITY), dir, "charge_density", namespace, gr%mesh, dtmp_2(:), fn_unit, ierr, geo=geo)
      SAFE_DEALLOCATE_A(dtmp_1)
      SAFE_DEALLOCATE_A(dtmp_2)
    end if
  ...
  subroutine output_medium_variables_electric(outp, namespace, dir, st, gr, hm, geo, st_elec, gr_elec, hm_elec)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_ELECTRIC)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_ELECTRIC), dir, fname, namespace, gr%mesh, D_field(:, idim), fn_unit, ierr, geo = geo)
      end do
      do ip = 1, gr%mesh%np
        e_susceptibility(ip) = sqrt(sum(D_field(ip, :)**2))/(P_ep*sqrt(sum(E_field(ip, :)**2))) - M_ONE
      end do
      write(fname, '(1a)') 'electric_susceptibility'
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_ELECTRIC), dir, fname, namespace, gr%mesh, e_susceptibility(:), fn_unit, ierr, geo = geo)
  ...
  subroutine output_medium_variables_magnetic(outp, namespace, dir, st, gr, hm, geo, st_elec, gr_elec, hm_elec)    !< have to set unit output correctly
    ...
    if(outp%what(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_MAGNETIC)) then
      ...
      do idim = 1, st%dim
        ...
        call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_MAGNETIC), dir, fname, namespace, gr%mesh, H_field(:, idim), fn_unit, ierr, geo = geo)
      end do
      do ip = 1, gr%mesh%np
        b_susceptibility(ip) = sqrt(sum(H_field(ip, :)**2))/(P_ep*sqrt(sum(B_field(ip,:)**2))) - M_ONE
      end do
      write(fname, '(1a)') 'magnetic_susceptibility'
      call dio_function_output(outp%how(OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_MAGNETIC), dir, fname, namespace, gr%mesh, b_susceptibility(:), fn_unit, ierr, geo = geo)
