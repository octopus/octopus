...
contains
  ...
  ! ---------------------------------------------------------
  subroutine output_all(outp, namespace, dir, gr, geo, iter, st, hm, ks)
    ...
    if(outp%what(OPTION__OUTPUT__MESH_R) .and. (iter == -1 .or. mod(iter, outp%output_interval(OPTION__OUTPUT__MESH_R)) == 0)) then
      do idir = 1, gr%sb%dim
        ...
        call dio_function_output(outp%how(OPTION__OUTPUT__MESH_R), dir, fname, namespace, gr%mesh, gr%mesh%x(:,idir), &
    ...
    do iout = 1, 40
      if(bitand(outp%how(iout), OPTION__OUTPUTFORMAT__ETSF)) then
        call output_etsf(outp, namespace, dir, st, gr, geo, iter)
        exit
      end if
    end do
      ...
      if (ks%oep%level == XC_OEP_FULL) then
        if (ks%oep%has_photons) then
          if(outp%what(OPTION__OUTPUT__PHOTON_CORRELATOR)) then
            ...
            call dio_function_output(outp%how(OPTION__OUTPUT__PHOTON_CORRELATOR), dir, trim(fname), namespace, gr%mesh, ks%oep%pt%correlator(:,1), &
              units_out%length, ierr, geo = geo)
          end if
        end if
      end if
  ...
  subroutine output_localization_funct(outp, namespace, dir, st, hm, gr, geo)
    ...
    if(outp%what(OPTION__OUTPUT__ELF) .or. outp%what(OPTION__OUTPUT__ELF_BASINS)) then
      ...
      if(outp%what(OPTION__OUTPUT__ELF)) then
        ...
        call dio_function_output(outp%how(OPTION__OUTPUT__ELF), dir, trim(fname), namespace, gr%mesh, &
    ...
    if(outp%what(OPTION__OUTPUT__BADER)) then
      do is = 1, st%d%nspin
        ...
        call dio_function_output(outp%how(OPTION__OUTPUT__BADER), dir, trim(fname), namespace, gr%mesh, &
    ...
    if(outp%what(OPTION__OUTPUT__EL_PRESSURE)) then
      ...
      call dio_function_output(outp%how(OPTION__OUTPUT__EL_PRESSURE), dir, "el_pressure", namespace, gr%mesh, &
  ...
  contains
    ...
    subroutine out_basins(ff, filename)
      ...
      call dio_function_output(outp%how(1), dir, trim(filename), namespace, gr%mesh, &
  ...
  subroutine output_energy_density(outp, namespace, dir, hm, ks, st, geo, gr)
    ...
    if(outp%what(OPTION__OUTPUT__ENERGY_DENSITY)) then
      ...
      case(UNPOLARIZED)
        ...
        call dio_function_output(outp%how(OPTION__OUTPUT__ENERGY_DENSITY), dir, trim(fname), namespace, gr%mesh, &
          ...
          do idim = 1, st%d%dim
            ...
            if(has_phase) then
              if(.not. this%basis%submesh) then
                ! MFT: TODO: which how should be passed here?
               call zio_function_output(outp%how(1), dir, fname, namespace, mesh, &
               ...
               ! MFT: TODO: which how should be passed here?
               call zio_function_output(outp%how(1), dir, fname, namespace, mesh, tmp, fn_unit, ierr, geo = geo)
              end if
            else
              if(.not.this%basis%submesh) then
                if (states_are_real(st)) then
                  ! MFT: TODO: which how should be passed here?
                  call dio_function_output(outp%how(1), dir, fname, namespace, mesh, &
                ...
                if (states_are_real(st)) then
                  ...
                  ! MFT: TODO: which how should be passed here?
                  call dio_function_output(outp%how(1), dir, fname, namespace, mesh, dtmp, fn_unit, ierr, geo = geo)
                else
                  tmp = M_Z0
