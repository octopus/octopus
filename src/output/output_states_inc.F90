...
subroutine output_states(outp, namespace, dir, st, gr, geo, hm)
  ...
  if(outp%what(OPTION__OUTPUT__DENSITY)) then
    ...
    do is = 1, st%d%nspin
      ...
      call dio_function_output(outp%how(OPTION__OUTPUT__DENSITY), dir, fname, namespace, gr%fine%mesh, &
  ...
  if(outp%what(OPTION__OUTPUT__POL_DENSITY)) then
    ...
    do is = 1, st%d%nspin
      ...
      call io_function_output_vector(outp%how(OPTION__OUTPUT__POL_DENSITY), dir, fname, namespace, gr%fine%mesh, polarization, gr%sb%dim, fn_unit, ierr, &
        ...
        do ik = st%d%kpt%start, st%d%kpt%end
          do idim = 1, st%d%dim
            ...
            if (states_are_real(st)) then
              ...
              call dio_function_output(outp%how(OPTION__OUTPUT__WFS), dir, fname, namespace, gr%mesh, dtmp, &
                fn_unit, ierr, geo = geo)
            else
              call states_elec_get_state(st, gr%mesh, idim, ist, ik, ztmp)
              call zio_function_output(outp%how(OPTION__OUTPUT__WFS), dir, fname, namespace, gr%mesh, ztmp, &
                fn_unit, ierr, geo = geo)
            end if
          end do
        end do
      ...
      if(loct_isinstringlist(ist, outp%wfs_list)) then
        do ik = st%d%kpt%start, st%d%kpt%end
          do idim = 1, st%d%dim
            ...
            call dio_function_output (outp%how(OPTION__OUTPUT__WFS_SQMOD), dir, fname, namespace, gr%mesh, &
  ...
  if(outp%what(OPTION__OUTPUT__KINETIC_ENERGY_DENSITY)) then
    ...
    case(UNPOLARIZED)
      write(fname, '(a)') 'tau'
      call dio_function_output(outp%how(OPTION__OUTPUT__KINETIC_ENERGY_DENSITY), dir, trim(fname), namespace, gr%mesh, &
