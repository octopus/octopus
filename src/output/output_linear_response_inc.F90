...
! ---------------------------------------------------------
subroutine X(output_lr) (outp, namespace, space, dir, st, gr, lr, idir, isigma, ions, pert_unit)
  ...
  if(isigma == 1) then ! the density, current, etc. are only defined for the + frequency
    ...
    if(bitand(outp%what, OPTION__OUTPUT__DENSITY) /= 0) then
      ...
      do is = 1, st%d%nspin
        ...
        call X(io_function_output)(outp%how, dir, fname, namespace, space, gr%mesh, lr%X(dl_rho)(:, is), &
          fn_unit / pert_unit, ierr, ions = ions)
      end do
    end if
    ...
    if(bitand(outp%what, OPTION__OUTPUT__POL_DENSITY) /= 0) then
      ...
      do is = 1, st%d%nspin
        do idir2 = 1, space%dim
          ...
          call X(io_function_output)(outp%how, dir, fname, namespace, space, &
            gr%mesh, tmp, fn_unit / pert_unit, ierr, ions = ions)
        end do
      end do
      ...
      if(states_are_complex(st)) then
        ...
        do is = 1, st%d%nspin
          do idir2 = 1, gr%sb%dim
            ...
            call zio_function_output(outp%how, dir, fname, namespace, space, &
              gr%mesh, lr%dl_j(:, idir2, is), fn_unit / pert_unit, ierr, ions = ions)
          end do
        end do
      else
      ...
      if(loct_isinstringlist(ist, outp%wfs_list)) then
        do ik = st%d%kpt%start, st%d%kpt%end
          do idim = 1, st%d%dim
            ...
            call X(io_function_output) (outp%how, dir, fname, namespace, space, gr%mesh, &
              lr%X(dl_psi) (1:, idim, ist, ik), fn_unit  / pert_unit, ierr, ions = ions)
          end do
        end do
      end if
    end do
...
contains
  ...
  subroutine lr_elf(filename1, filename2)
    ...
    do is = 1, st%d%nspin
      ...
      call X(io_function_output)(outp%how, dir, trim(fname), namespace, space, &
        gr%mesh, lr%X(dl_de)(1:gr%mesh%np,is), unit_one / pert_unit, ierr, ions = ions)
    end do
    ...
    do is = 1, st%d%nspin
      ...
      call X(io_function_output)(outp%how, dir, trim(fname), namespace, space, &
        gr%mesh, lr%X(dl_elf)(1:gr%mesh%np,is), unit_one / pert_unit, ierr, ions = ions)
    end do
