...
program hs_from_acc
  ...
  call loct_parse_float(check_inp('SpecEnergyStep'), CNST(0.05)/units_inp%energy%factor, s%energy_step)
  call loct_parse_float(check_inp('SpecMinEnergy'),  M_ZERO, s%min_energy)
  call loct_parse_float(check_inp('SpecMaxEnergy'),  M_ONE     /units_inp%energy%factor, s%max_energy)
