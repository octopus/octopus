...
contains
  ...
  subroutine crystal_init(al, ntype, natom, maxnatom, coorat, nk_axis, k_shift, nrk, nrkmax, rk, w, &
    ...
    write(message(1), '(a)') " direct space matrix (some units)"
    write(message(2), '(1x,3f10.5)') (amet(1,j),j=1,3)
    write(message(3), '(1x,3f10.5)') (amet(2,j),j=1,3)
    write(message(4), '(1x,3f10.5)') (amet(3,j),j=1,3)
    call write_info(4)

    write(message(1), '(a)') " reciprocal space matrix (some units)"
    write(message(2), '(1x,3f10.5)') (bmet(1,j),j=1,3)
    write(message(3), '(1x,3f10.5)') (bmet(2,j),j=1,3)
    write(message(4), '(1x,3f10.5)') (bmet(3,j),j=1,3)
    call write_info(4)
    ...
    do i = 1, nrk
      ...
      ! printout
      write(message(1),9060)i,w(i),(rk(j,i),j=1,3), (real(neg, REAL_PRECISION)*rkcar(j),j=1,3)
9060  format(i4,3x,f10.6,3x,3f10.6,3x,3f10.6)
    ...
    write(message(1),'(a)')
    write(message(2),'(1x,i3,a)') nrk, ' k points generated by program from parameters :'
    write(message(3),'(1x,a)') '---------------------------------------------------'
    write(message(4),'(4x,a,3i5,6x,a,3f6.2)') 'n =', nx, ny, nz, 's = ', sx, sy, sz
    write(message(5),'(a)')
    call write_info(5)
...
!
         if(ipr == 1) then
            write(message(1), '(a)')
            write(message(2), '(4x,a)') 'Rotation matrices (r-lattice) and fractional translations (r-lattice)'
            write(message(3), '(a)')
            call write_info(3)
!
            do i = 1, ntrans
               write(message(1), '(i5,3(3x,3i3),4x,3f9.5,1x,a5)') i, ((rmtrx(i,j,k),k=1,3),j=1,3),           &
                    (tnp(i,k),k=1,3), id(i)
               call write_info(1)
!
            end do

            write(message(1),'(a)') ' skipping symmetry check!'
            call write_info(1)
         end if
...
!            call write_info(1)
               write(message(1), '(a)')
               write(message(2), '(a,i3,a,i3,a)') ' symmetry operations',j,' and',i,' are equal'
               call write_info(2)
            end if
            ierr = i
...
!
      if(ipr == 1) then
         ...
         if ((ihg == 7.and.nca == 24).or.(ihg == 5.and.nca == 48)) then
            write(message(2),'(a, a12, a)') ' The point group of the crystal is the full ', cst(ihg), 'group'
            call write_info(2)
         else
            write(message(2),'(a, a12, a)') ' The crystal system is ',cst(ihg),' with operations: '
            if (nc .le. 24) then
               write(message(3),'(5x,24i3)') (ic(i),i=1,nc)
               call write_info(3)
            else
               write(message(3),'(5x,24i3)') (ic(i),i=1,24)
               write(message(4),'(5x,24i3)') (ic(i),i=25,nc)
               call write_info(4)
            endif
         end if
      end if
