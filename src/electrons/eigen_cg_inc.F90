...
subroutine X(eigensolver_cg) (namespace, gr, st, hm, xc, pre, tol, niter, converged, ik, diff, orthogonalize_to_all, &
  ...
  eigenfunction_loop : do ist = converged + 1, st%nst
    ...
    iter_loop: do iter = 1, maxter
      ...
      if (iter > 1 .and. residue > 10*tol) then
        ! exit loop if residue is 2 orders of magnitude smaller than initial
        if (residue/residue_initial < 0.01) exit iter_loop
        ! exit loop if change in residue is small
        if (abs(residue-residue_previous)/residue_previous < 0.01) then
