  ...
  iteration: do iter = 1, maxiter-1 ! One iteration was used up to get initial Ritz-vectors.
    ...
    if(iter > 1) then
      ...
      if(no_bof) then
        ...
        call profiling_in(C_PROFILING_LOBPCG_INV, TOSTRING(X(LOBPCG_INV)))
    ...
    call profiling_in(C_PROFILING_LOBPCG_ESOLVE, TOSTRING(X(LOBPCG_ESOLVE)))
    no_bof = .false.
...
contains
  ...
  subroutine X(lobpcg_apply_constraints)(vs_start, vs, nidx, idx)
    ...
    call profiling_in(prof, TOSTRING(X(LOBPCG_CONSTRAINTS)))
