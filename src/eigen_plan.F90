...
subroutine eigen_solver_plan(m, st, hamilt, tol, niter, converged, diff)
  ...
  k_points : do ik = 1, st%nik
    ...
    outer_loop : do
      ...
      call X(lalg_copy)(n*winsiz, eigenvec(1, nec + 1), v(1, 1))
      ...
      inner_loop: do
        ...
        if ( d2+1.le.krylov .and. matvec.lt.maxmatvecs) then
          ...
          if(res(nec+1)<tol) then
            do i = 2, winsiz
              call X(lalg_gemv)(n, d2, R_TOTYPE(M_ONE), v(1, 1), hevec(1, i), &
                   R_TOTYPE(M_ZERO), eigenvec(1, nec+i))
            enddo
            do i = 2, winsiz
              call X(lalg_gemv)(n, d2, R_TOTYPE(M_ONE), av(1, 1), hevec(1, i), &
        ...
        ordering: do i = nec + 1, nec+winsiz-1
          ...
          do j = i, 2, -1
            ...
            call dlalg_swap(1, eigenval(j-1), eigenval(j))
            call dlalg_swap(1, res(j-1), res(j))
            call X(lalg_swap)(n, eigenvec(1,j), eigenvec(1,j-1))
          enddo
        enddo ordering
