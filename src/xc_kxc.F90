...
subroutine xc_get_kxc(xcs, m, rho, ispin, kxc)
  ...
  space_loop: do i = 1, m%np
    ...
    functl_loop: do ixc = 1, 2
      ...
      case(XC_FAMILY_LDA)
        call xc_lda_kxc(functl(ixc)%conf, l_dens(1), l_dedd(1,1,1))
...
contains
  ...
  subroutine lda_init()
    ...
    allocate(dens(m%np, spin_channels), dedd(m%np, spin_channels, spin_channels,spin_channels))
    allocate(l_dens(spin_channels), l_dedd(spin_channels, spin_channels, spin_channels))
    dedd = M_ZERO
