...
contains
  ...
  subroutine casida_run(sys, hm, fromScratch)
    ...
    call parse_variable(dummy_parser, 'CasidaTheoryLevel', CASIDA_EPS_DIFF + CASIDA_PETERSILKA + CASIDA_CASIDA, theorylevel)
    ...
    !% valid.
    !%End
    call parse_variable(dummy_parser, 'CasidaTransitionDensities', "0", cas%trandens)
    ...
    if(parse_block(sys%parser, 'CasidaMomentumTransfer', blk)==0) then
      ...
      !%End
      call parse_variable(dummy_parser, 'CasidaQuadratureOrder', 5, cas%avg_order)
    else
      cas%qvector(:) = M_ZERO
      cas%qcalc = .false.
    end if
    ...
    if(sys%st%d%ispin == UNPOLARIZED) then
      call parse_variable(dummy_parser, 'CasidaCalcTriplet', .false., cas%triplet)
    else
      cas%triplet = .false.
    end if
    ...
    !%End
    call parse_variable(dummy_parser, 'CasidaHermitianConjugate', .false., cas%herm_conj)
    ...
    !% valid.
    !%End
    call parse_variable(dummy_parser, 'CasidaPrintExcitations', "all", cas%print_exst)
    ...
    !%End
    call parse_variable(dummy_parser, 'CasidaWeightThreshold', -M_ONE, cas%weight_thresh)
    ...
    !%End
    call parse_variable(dummy_parser, 'CasidaCalcForces', .false., cas%calc_forces)
    if(cas%calc_forces) then
      ...
      !%End
      call parse_variable(dummy_parser, 'CasidaCalcForcesKernel', .true., cas%calc_forces_kernel)
      ...
      !%End
      call parse_variable(dummy_parser, 'CasidaCalcForcesSCF', .false., cas%calc_forces_scf)
    end if
