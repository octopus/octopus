...
program octopus
  ...
  !% using.
  !%End
  call parse_variable(namespace, 'ReportMemory', .false., conf%report_memory)
  ...
  !%End
  if(parse_block(namespace, 'CalculationMode', blk) == 0) then
  ...
  call parse_variable(namespace, 'CalculationMode', CM_GS, inp_calc_mode)
  ...
  ! now we really start
  call run(namespace, inp_calc_mode)
