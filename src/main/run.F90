...
contains
  ...
  subroutine run(namespace, cm)
    ...
    do while (iter%has_next())
      sys => iter%get_next()
      select type (sys)
      ! still use old propagators for Maxwell until multi-system propagators are implemented
    ...
    if(all_sys_abstract) then
      ...
      do while (iter%has_next())
        ...
        ! still use old propagators for Maxwell until multi-system propagators are implemented, see above
