...
contains
  ...
  subroutine transform_states(st, restart, gr, prefix)
    ...
    !%End
    if(parse_isdef(datasets_check(trim(block_name))) /= 0) then
      if(parse_block(datasets_check(trim(block_name)), blk) == 0) then
        ...
        ! states_copy with exclude will not allocate this, but it will be used in states_look_and_load
        SAFE_ALLOCATE(stin%occ(1:st%nst, 1:st%d%nik))
        ...
        call messages_info(1)
        call input_error(trim(block_name))
      end if
    end if
