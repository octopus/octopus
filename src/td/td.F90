...
contains
  ...
  contains
    ...
    subroutine init_wfs()
      ...
      if(gr%der%boundaries%spiralBC) then
        ...
        if(fromScratch) call states_elec_fermi(st, gr%mesh) 
      end if
      ...
      call states_elec_fermi(st, gr%mesh, compute_spin = .not. gr%der%boundaries%spiralBC)
  ...
  subroutine td_load(restart, namespace, gr, st, hm, td, ierr)
    ...
    if(gr%der%boundaries%spiralBC) then
      ...
      if(err /= 0) call states_elec_fermi(st, gr%mesh)
    end if
