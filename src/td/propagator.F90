...
contains
  ...
  subroutine propagator_init(gr, parser, st, tr, have_fields, family_is_mgga)
    ...
    call parse_variable(dummy_parser, 'TDPropagator', PROP_ETRS, tr%method)
    ...
    call parse_variable(dummy_parser, 'TDStepsWithSelfConsistency', 0, tr%scf_propagation_steps)
    ...
    !%End
    call parse_variable(dummy_parser, 'TDSCFThreshold', CNST(1.0e-6), tr%scf_threshold)
