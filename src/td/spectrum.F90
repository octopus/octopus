...
contains
  ...
  subroutine spectrum_cross_section(in_file, out_file, spectrum, ref_file)
    ...
    call spectrum_add_pcm_dipole(dipole, pcm_local_field, time_steps, dt)
    ...
    if ( pcm_local_field ) then ! only applicable in the case of local field effects within PCM
      ...
      do ie = 0, no_e
        call pcm_eps(eps(ie), ie*spectrum%energy_step)
  ...
  subroutine spectrum_add_pcm_dipole(dipole,localf,time_steps,dt)
    FLOAT,   intent(inout) :: dipole(0:, :, :)
    logical, intent(out)   :: localf
    ...
    ! unit io variables
    integer :: asc_unit_test, asc_gs_unit
    integer :: cavity_unit
    ...
    ! parsing the local field flag from input
    call parse_variable('PCMLocalField', .false., pcm%localf)
    localf = pcm%localf

    ! the following only applies to local field case
    if ( .not.pcm%localf ) then
      POP_SUB(spectrum_add_pcm_dipole)
      return
    endif

    ! FIXME: write to standard output file for cross-section spectrum
    write(*,*) "Cross-section spectrum contains all local field effects"
    ...
    write(cavity_unit,*) pcm%n_tesserae
    write(cavity_unit,*)
    do ia = 1, pcm%n_tesserae
     read(asc_unit_test,*) pcm%tess(ia)%point(1:3), aux_float, aux_int
     write(cavity_unit,*) 'H', pcm%tess(ia)%point(1:3)
    end do
    call io_close(asc_unit_test)
    call io_close(cavity_unit)

    ! FIXME: including the contribution of nuclei to the PCM polarization charges to be more general
    ! N.B. assuming time-dependent PCM polarization charges due to solute nuclei fixed in time

    ! FIXME: see if we can omit altogether this reading of ground state polarization charges
    ! reading ground state PCM polarization charges due to solute electrons
    ! the file to be read needs to be pulled-out from the pcm standard directory of ground state calculation and copied
    ! in equivalent directory for the time-propagation calculation with the proper change of names
    asc_gs_unit = io_open(PCM_DIR//'ASC_e_gs.dat', action='read')
    do ia = 1, pcm%n_tesserae
     read(asc_gs_unit,*) ( aux_vec(ii), ii=1, 3), pcm%q_e(ia), aux_int
    end do
    call io_close(asc_gs_unit)

    ! computing the dipole generated by the ground state PCM polarization charges due to solute electrons
    ! fixing convention of signs within Octopus (see src/hamiltonian/pcm:pcm_elect_energy and src/hamiltonian/epot:epot_init)
    ! N.B. now electrons has -1 charge
    call pcm_dipole(dipole_pcm(1:3), -pcm%q_e(1:pcm%n_tesserae), pcm%tess, pcm%n_tesserae)

    ! reading the time-dependent PCM polarization charges due to solute electrons
    ! computing the real-time dipole generated by the time-dependent PCM polarization charges due to solute electrons
    ! adding it to the real-time molecular dipole
    ! writing the total dipole and the PCM contribution to debug-purpose files

    ! opening time-dependent PCM charges standard and debug-purpose file
    asc_vs_t_unit = io_open(PCM_DIR//'ASC_e_vs_t.dat', action='read')
    asc_vs_t_unit_check = io_open(PCM_DIR//'ASC_e_vs_t_check.dat', action='write')

    ! opening time-dependent PCM and total dipole debug-purpose files
    dipole_vs_t_unit_check = io_open(PCM_DIR//'dipole_e_vs_t_check.dat', action='write')
    dipole_vs_t_unit_check1 = io_open(PCM_DIR//'dipole_e_vs_t_check1.dat', action='write')

    ! reading PCM charges for the zeroth-iteration
    read(asc_vs_t_unit,*) aux_float1, ( pcm%q_e_in(ia) , ia=1,pcm%n_tesserae )

    ! writing the ground state PCM dipole for debug purposes
    write(dipole_vs_t_unit_check,*) aux_float1, dipole_pcm

    ! getting varible not in calling
    nspin = size(dipole,3)

    dipole(0, 1, 1:nspin) = dipole(0, 1, 1:nspin) + dipole_pcm(1)
    dipole(0, 2, 1:nspin) = dipole(0, 2, 1:nspin) + dipole_pcm(2)
    dipole(0, 3, 1:nspin) = dipole(0, 3, 1:nspin) + dipole_pcm(3)

    do it = 1, time_steps

      ! FIXME: strange reading due to the fact that charges are written more than once per timestep
      ! FIXME: the origin of this problem is in the subroutine src/hamiltonian/pcm.F90:pcm_calc_pot_rs

      ! the patch here should be effective, anyhow

      ! reading PCM charges
      if ( ABS( aux_float1 - it*dt ) / aux_float1 > 1e-6 ) then
       read(asc_vs_t_unit,*) aux_float, ( pcm%q_e(ia) , ia=1,pcm%n_tesserae )
      else
       aux_float = aux_float1
       pcm%q_e = pcm%q_e_in
      endif
      do
       ! q_e_in is auxiliar here
       read(asc_vs_t_unit,*) aux_float1, ( pcm%q_e_in(ia) , ia=1,pcm%n_tesserae )
       if ( aux_float1 /= aux_float .or. aux_float1 == it*dt ) exit
      end do
      ...
      ! a sanity check
      ! assuming kick, the first-iteration dipole should be equal to the zeroth-iteration one
      do ii=1, nspin
      if ( dipole(0, 1, ii) == dipole(1, 1, ii) .and. &
           dipole(0, 2, ii) == dipole(1, 2, ii) .and. &
           dipole(0, 3, ii) == dipole(1, 3, ii)       ) then
       write(*,*) "sanity check: success. The first-iteration dipole is equal to the zeroth-iteration one"
      else
       write(*,*) "sanity check: failure. The first-iteration dipole is different from the zeroth-iteration one"
      endif
      enddo

      ! we always have kick for the optical spectrum - in Octopus (sic)
      ! in any case, the first-iteration dipole is made equal to the zeroth-iteration one by hand
      if ( it == 1 ) then
      ...
      write(asc_vs_t_unit_check,*) aux_float, ( pcm%q_e(ia) , ia=1,pcm%n_tesserae )
      write(dipole_vs_t_unit_check,*) aux_float, dipole_pcm
      write(dipole_vs_t_unit_check1,*) aux_float, dipole(it,:,1)
