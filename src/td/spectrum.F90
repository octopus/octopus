...
contains
  ...
  subroutine spectrum_init(spectrum, default_energy_step, default_max_energy)
    ...
    call parse_variable(dummy_parser, 'PropagationSpectrumType', SPECTRUM_ABSORPTION, spectrum%spectype)
    ...
    !%End
    call parse_variable(dummy_parser, 'SpectrumMethod', SPECTRUM_FOURIER, spectrum%method)
    ...
    if(spectrum%method == SPECTRUM_COMPRESSED_SENSING) then
      ...
      !%End
      call parse_variable(dummy_parser, 'SpectrumSignalNoise', CNST(0.0), spectrum%noise)
    ...
    call parse_variable(dummy_parser, 'PropagationSpectrumDampMode', default, spectrum%damp)
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumTransform', SPECTRUM_TRANSFORM_SIN, spectrum%transform)
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumStartTime',  M_ZERO, spectrum%start_time, units_inp%time)
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumEndTime', -M_ONE, spectrum%end_time, units_inp%time)
    ...
    call parse_variable(dummy_parser, 'PropagationSpectrumEnergyStep', fdefault, spectrum%energy_step, units_inp%energy)
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumMinEnergy', M_ZERO, spectrum%min_energy, units_inp%energy)
    ...
    call parse_variable(dummy_parser, 'PropagationSpectrumMaxEnergy', fdefault, spectrum%max_energy, units_inp%energy)
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumDampFactor', -M_ONE, spectrum%damp_factor, units_inp%time**(-1))
    ...
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumSigmaDiagonalization', .false., spectrum%sigma_diag)
...
100 continue
  ...
  subroutine spectrum_sigma_diagonalize(sigma, nspin, energy_step, min_energy, energy_steps, kick)
    ...
    !% This variable is only used if the cross_section_tensor is computed. 
    !%End
    call parse_variable(dummy_parser, 'PropagationSpectrumSymmetrizeSigma', .false., symmetrize)
