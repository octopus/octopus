...
contains
  ...
  subroutine spectrum_cross_section(in_file, out_file, spectrum, ref_file)
    ...
    logical :: pcm_run, pcm_local_field
    ...
    call spectrum_add_pcm_dipole(dipole, pcm_run, pcm_local_field, time_steps, dt, nspin)
    ...
    if ( pcm_local_field ) then
      ...
      call spectrum_times_pcm_epsilon(dipole, Imdipole, sigma, nspin, cmplxscl, spectrum, istart, iend, kick%time, dt, no_e)
    ...
    if( pcm_run ) &
      call spectrum_over_pcm_refraction_index(sigma, nspin, spectrum, no_e)
  ...
  subroutine spectrum_add_pcm_dipole(dipole, pcm_run, pcm_local_field, time_steps, dt, nspin)
    FLOAT,   intent(inout) :: dipole(0:, :, :)
    logical, intent(out)   :: pcm_run
    logical, intent(out)   :: pcm_local_field
    ...
    if ( pcm%localf ) then
     ...
     call messages_write('particularly, when static and high-frequency values of the dielectric functions are large (>~10).')
     call messages_new_line()
    ...
    do it = 1, time_steps
      ...
      !        the origin of this problem is in the subroutine src/hamiltonian/pcm.F90:pcm_calc_pot_rs
      ...
      do
       ! pcm%q_e_in is only auxiliary here
       read(asc_vs_t_unit,*) aux_float1, ( pcm%q_e_in(ia) , ia=1,pcm%n_tesserae )
  ...
  subroutine spectrum_times_pcm_epsilon(dipole, Imdipole, sigma, nspin, cmplxscl, spectrum, istart, iend, kick_time, dt, no_e)
    FLOAT, allocatable, intent(inout) :: sigma(:, :, :)
    FLOAT, allocatable, intent(in)    :: dipole(:, :, :), Imdipole(:, :, :)
    integer,            intent(in)    :: nspin
    ...
    call spectrum_fourier_transform(spectrum%method, SPECTRUM_TRANSFORM_SIN, spectrum%noise, &
      istart + 1, iend + 1, kick_time, dt, dipoleb, 1, no_e + 1, spectrum%energy_step, sigmab, spectrum%cmplxscl)
  ...
  subroutine spectrum_over_pcm_refraction_index(sigma, nspin, spectrum, no_e)
    FLOAT, allocatable, intent(inout) :: sigma(:, :, :)
    integer,            intent(in)    :: nspin
    type(spectrum_t),   intent(in)    :: spectrum !< check intent in
    integer,            intent(in)    :: no_e
