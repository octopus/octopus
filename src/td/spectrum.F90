...
contains
  ...
  subroutine spectrum_add_pcm_dipole(dipole, time_steps, dt, nspin)
    ...
    read(asc_vs_t_unit,fmt=trim(adjustl(asc_vs_t_unit_format))) aux_float1, ( pcm%q_e_in(ia) , ia=1,pcm%n_tesserae )

    do it = 1, time_steps

      ! reading real-time PCM charges due to electrons per timestep
      read(asc_vs_t_unit,fmt=trim(adjustl(asc_vs_t_unit_format))) aux_float, ( pcm%q_e(ia) , ia=1,pcm%n_tesserae )
      ...
      write(asc_vs_t_unit_check,fmt=trim(adjustl(asc_vs_t_unit_format))) aux_float, ( pcm%q_e(ia) , ia=1,pcm%n_tesserae )
