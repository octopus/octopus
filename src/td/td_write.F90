...
module td_write_oct_m
  ...
  type td_write_t
    ...
    type(td_write_prop_t) :: out_ldau(OUT_LDAU_MAX)
...
contains
  ...
  subroutine td_write_init(writ, gr, st, hm, geo, ks, ions_move, with_gauge_field, kick, iter, max_iter, dt, mc)
    ...
    if(mpi_grp_is_root(mpi_world)) then
    !  if(writ%out_ldau(OUT_LDAU_EFFECTIVEU)%write) &
    !    call write_iter_init(writ%out_ldau(OUT_LDAU_EFFECTIVEU)%handle, &
