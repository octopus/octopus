...
contains
  ...
  subroutine td_write_init(writ, namespace, outp, gr, st, hm, geo, ks, ions_move, with_gauge_field, kick, iter, max_iter, dt, mc)
    ...
    if(all(outp%how == 0) .and. writ%out(OUT_N_EX)%write) call io_function_read_what_how_when(gr%sb, namespace, outp%what, outp%how, outp%output_interval)
  ...
  subroutine td_write_output(namespace, gr, st, hm, ks, outp, geo, iter, dt)
    ...
    call output_all(outp, namespace, filename, gr, geo, iter, st, hm, ks)
    if(present(dt)) then
