...
contains
  ...
  subroutine td_cfmagnus4(ks, namespace, hm, gr, st, tr, time, dt, ions, geo, iter)
    ...
    call hamiltonian_elec_update2(hm, gr%mesh, gr%der%boundaries, (/ t1, t2 /), (/ M_TWO * alpha2, M_TWO * alpha1/) )
    ! propagate by dt/2 
    call propagation_ops_elec_exp_apply(tr%te, st, gr%mesh, hm, M_HALF*dt)

    hm%vhxc = M_TWO * (alpha1 * vhxc1 + alpha2 * vhxc2)
    call hamiltonian_elec_update2(hm, gr%mesh, gr%der%boundaries, (/ t1, t2 /), (/ M_TWO * alpha1, M_TWO * alpha2/) )
    ! propagate by dt/2
