...
contains
  ...
  subroutine propagator_elec_nullify(this)
    type(propagator_elec_t), intent(out) :: this
  ...
  subroutine propagator_elec_copy(tro, tri)
    type(propagator_elec_t), intent(inout) :: tro
    type(propagator_elec_t), intent(in)    :: tri
  ...
  subroutine propagator_elec_dt_bo(scf, namespace, gr, ks, st, hm, geo, mc, outp, iter, dt, ions, scsteps)
    ...
    if(hm%lda_u_level /= DFT_U_NONE) then
      call messages_not_implemented("DFT+U with propagator_elec_dt_bo")  
    end if
