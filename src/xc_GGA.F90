...
subroutine xc_gga(xcs, m, st, vxc, ex, ec, ip, qtot)
  ...
  call la_copy(m%np*st%nspin, st%rho(1, 1), 1, d(1, 1), 1)
  ...
  if(xcs%nlcc) then
    do is = 1, st%spin_channels
      call la_axpy(m%np, M_ONE/st%spin_channels, st%rho_core(1), 1, d(1, is), 1)
    end do
  end if
  ...
  do is = 1, st%spin_channels
    ...
    call la_axpy(m%np, -M_ONE, rhoplus(1), 1, lpot(1, is), 1)
  end do
