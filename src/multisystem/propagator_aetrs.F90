...
contains
  ...
  ! ---------------------------------------------------------
  function propagator_aetrs_constructor(dt, predictor_corrector) result(this)
    FLOAT,                     intent(in) :: dt
    ...
    if(predictor_corrector) then
      ...
      call this%add(AETRS_EXTRAPOLATE_HAMILTONIAN) ! extrapolate Hamiltonian H_t3 from H(t) and H(t-dt)
