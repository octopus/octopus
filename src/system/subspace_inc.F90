...
subroutine X(subspace_projections)(mesh, st, gs_st, ik, proj)
  ...
  PUSH_SUB(X(subspace_diag_hamiltonian))
  call profiling_in(hamiltonian_prof, "SUBSPACE_PROJECTIONS")

  if(states_are_packed(st) .and. accel_is_enabled()) then
   message(1) = "Subspace_projections is not implemented with packed states or accel."
   call messages_fatal(1) 
  else
...
end subroutine X(subspace_projections)
