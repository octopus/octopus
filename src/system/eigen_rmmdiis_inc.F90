...
subroutine X(eigensolver_rmmdiis) (gr, st, hm, pre, tol, niter, converged, ik, diff, blocksize)
  ...
  do minst = st%st_start, st%st_end, blocksize
    ...
    do iter = 1, niter
      call batch_init(psib(iter), st%d%dim, maxst - minst + 1 - sum(done))
      call batch_init(resb(iter), st%d%dim, maxst - minst + 1 - sum(done))
    ...
    do ist = minst, maxst
      ...
      fr(1, ii) = X(mf_dotp)(gr%mesh, st%d%dim, psi(:, :, 2, ii), psi(:, :, 2, ii), reduce = .false.)
      fr(2, ii) = X(mf_dotp)(gr%mesh, st%d%dim, psi(:, :, 1, ii), psi(:, :, 2, ii), reduce = .false.)
      fr(3, ii) = X(mf_dotp)(gr%mesh, st%d%dim, psi(:, :, 2, ii), res(:, :, 2, ii), reduce = .false.)
      fr(4, ii) = X(mf_dotp)(gr%mesh, st%d%dim, psi(:, :, 1, ii), res(:, :, 2, ii), reduce = .false.)
    end do
    ...
    do ist = minst, maxst
      ...
      lambda(ii) = CNST(2.0)*cc/(cb + sqrt(cb**2 - CNST(4.0)*ca*cc))

      ! restrict the value of lambda to be between 0.1 and 1.0
      if(abs(lambda(ii)) > CNST(1.0)) lambda(ii) = lambda(ii)/abs(lambda(ii))
      if(abs(lambda(ii)) < CNST(0.1)) lambda(ii) = CNST(0.1)*lambda(ii)/abs(lambda(ii))
    end do
    ...
    do iter = 2, niter
      ...
      do ist = minst, maxst
        ...
        forall(idim = 1:st%d%dim, ip = 1:gr%mesh%np)
          psi(ip, idim, iter, ii) = lambda(ii)*psi(ip, idim, iter, ii) + psi(ip, idim, iter - 1, ii)
        end forall
      end do
      ...
      do ist = minst, maxst
        ...
        if(done(ii) /= 0 .and. .not. failed(ii)) cycle

        do idim = 1, st%d%dim
          call lalg_axpy(gr%mesh%np, -st%eigenval(ist, ik), psi(:, idim, iter, ii), res(:, idim, iter, ii))
        end do
      ...
      do ist = minst, maxst
        ...
        failed(ii) = .false.
        call lalg_lowest_geneigensolve(1, iter, mm(:, :, 1, ii), mm(:, :, 2, ii), eval, evec, bof = failed(ii))
        if(failed(ii)) then
          last(ii) = iter - 1
          cycle
        end if
        ...
        ! slower).
        do idim = 1, st%d%dim
          call lalg_scal(gr%mesh%np, evec(iter, 1), psi(:, idim, iter, ii))
          call lalg_scal(gr%mesh%np, evec(iter, 1), res(:, idim, iter, ii))
        end do

        do jj = 1, iter - 1
          do idim = 1, st%d%dim
            call lalg_axpy(gr%mesh%np, evec(jj, 1), psi(:, idim, jj, ii), psi(:, idim, iter, ii))
            call lalg_axpy(gr%mesh%np, evec(jj, 1), res(:, idim, jj, ii), res(:, idim, iter, ii))
          end do
        end do
    ...
    do ist = minst, maxst
      ...
      if(.not. failed(ii)) then
        ...
        forall (idim = 1:st%d%dim, ip = 1:gr%mesh%np)
          st%X(psi)(ip, idim, ist, ik) = psi(ip, idim, iter - 1, ii) + lambda(ii) * st%X(psi)(ip, idim, ist, ik)
        end forall
      else
        do idim = 1, st%d%dim
