...
contains
  ...
  subroutine pert_init(this, parser, pert_type, gr, geo)
    ...
    if ( this%pert_type == PERTURBATION_MAGNETIC ) then
      ...
      call parse_variable(dummy_parser, 'MagneticGaugeCorrection', GAUGE_GIPAW, this%gauge)
    ...
    if(this%pert_type == PERTURBATION_KDOTP) then
      ...
      call parse_variable(dummy_parser, 'KdotPUseNonLocalPseudopotential', .true., this%use_nonlocalpps)
      ...
      !%End
      call parse_variable(dummy_parser, 'KdotPVelMethod', OPTION__KDOTPVELMETHOD__GRAD_VEL, this%vel_method)
    end if
