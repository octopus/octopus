...
contains
  ...
  subroutine exponential_init(te)
    ...
    !%End
    call parse_variable(dummy_parser, 'TDExponentialMethod', EXP_TAYLOR, te%exp_method)
    ...
    case(EXP_LANCZOS)
      ...
      !% wrong.
      !%End
      call parse_variable(dummy_parser, 'TDLanczosTol', CNST(1e-5), te%lanczos_tol)
    ...
    if(te%exp_method==EXP_TAYLOR.or.te%exp_method==EXP_CHEBYSHEV.or.te%exp_method==EXP_LANCZOS) then
      ...
      !%End
      call parse_variable(dummy_parser, 'TDExpOrder', DEFAULT__TDEXPORDER, te%exp_order)
    ...
    if(te%exp_method == EXP_LANCZOS) then
      ...
      !%End
      call parse_variable(dummy_parser, 'ArnoldiOrthogonalization', OPTION__ARNOLDIORTHOGONALIZATION__CGS, &
                              te%arnoldi_gs)
    end if
