    ...
    do ist = st%st_start, st%st_end
      do idim = 1, st%d%dim
        ...
        ! conveniently, multigrid_coarse2fine sets the boundary conditions in the process
          call X(multigrid_coarse2fine)(gr%fine%tt, gr%der, gr%mesh, gr%fine%mesh, st%X(psi)(:, idim, ist, ik), psi(:, idim))
          if (present(lr)) then
            call X(multigrid_coarse2fine)(gr%fine%tt, gr%der, gr%mesh, gr%fine%mesh, &
