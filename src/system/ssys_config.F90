...
contains
  ...
  subroutine ssys_config_parse_hamiltonian(this, nspin)
    ...
    SAFE_ALLOCATE(cnfg)
    call json_init(cnfg)
    call json_set(trms, "tnadd", cnfg)
    SAFE_ALLOCATE(defn)
    call ssys_config_parse_tnadd(defn, nspin)
    ...
    SAFE_ALLOCATE(defn)
    call ssys_config_parse_external(defn)
    call json_set(cnfg, "definition", defn)
    nullify(cnfg, defn)
    SAFE_ALLOCATE(cnfg)
    call json_init(cnfg)
    ...
    SAFE_ALLOCATE(defn)
    call ssys_config_parse_ionic(defn)
    call json_set(cnfg, "definition", defn)
    nullify(trms, cnfg, defn)
