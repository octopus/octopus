...
subroutine X(output_me_transition_dipole) (dir, gr, st, list_ij)
  character(len=*), intent(in)    :: dir
  type(grid_t),     intent(inout) :: gr
  type(states_t),   intent(in)    :: st
  integer       ,   intent(in)    :: list_ij(:,:)
  ...
  PUSH_SUB(X(output_me_transition_dipole))

  npairs = size(list_ij(:,1))
  ...
  call io_close(iunit)
  POP_SUB(X(output_me_transition_dipole))
end subroutine X(output_me_transition_dipole)
