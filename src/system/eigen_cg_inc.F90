...
subroutine X(eigensolver_cg2) (namespace, gr, st, hm, xc, pre, tol, niter, converged, ik, diff, orthogonalize_to_all, &
  ...
  eigenfunction_loop : do ist = converged + 1, st%nst
    ...
    call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, psi, h_psi, ist, ik)

    if(fold_) then
      call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, h_psi, psi2, ist, ik)
      ! h_psi = (H-shift)^2 psi
      do idim = 1, st%d%dim
    ...
    iter_loop: do iter = 1, maxter
      ...
      call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, cg, h_cg, ist, ik)

      if(fold_) then
        call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, h_cg, psi2, ist, ik)
        ! h_psi = (H-shift)^2 psi
        do idim = 1, st%d%dim
...
subroutine X(eigensolver_cg2_new) (namespace, gr, st, hm, tol, niter, converged, ik, diff)
  ...
  states: do ist = conv + 1, nst
    ...
    ! Calculate starting gradient: |hpsi> = H|psi>
    call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, psi, phi, ist, ik)
    niter = niter + 1
    ...
    band: do i = 1, maxter - 1 ! One operation has already been made.
      ...
      call X(hamiltonian_elec_apply)(hm, namespace, gr%mesh, cgp, hcgp, ist, ik)
