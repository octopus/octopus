...
!!
  ...
  subroutine output_mxll_init(outp, namespace, sb)
    ...
    !% Must be >= 0. If it is 0, then no output is written. 
    !%End
    call parse_variable('MaxwellOutputInterval', 50, outp%output_interval)
    if(outp%output_interval < 0) then
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__CURRENT) /= 0) then
      outp%wfs_list = trim("1-3")
    end if
  ...
  ! ---------------------------------------------------------
  subroutine output_mxll(outp, gr_mxll, st_mxll, hm_mxll, time, geo, dir, gr, st_elec, hm_elec)
    ...
    call output_charge_density_mxll(st_mxll, gr_mxll, hm_mxll, geo, dir, outp, st=st_elec, gr=gr_elec, hm=hm_elec)
    ...
    call output_test_output(st_mxll, gr_mxll, geo, dir, outp)
  ...
  subroutine output_mxll_free(outp, gr, st, hm, time, geo, dir)
    ...
    call output_test_output(st, gr, geo, dir, outp)
  ...
  subroutine output_states_mxll(st, gr, hm, geo, dir, outp)
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__ELECTRIC_FIELD) /= 0) then
      ...
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np_part,1:st%d%dim))
      if (.not. hm%ma_mx_coupling) then
        call get_electric_field_state(st%rs_state_trans+st%rs_state_long, dtmp, &
                                        st%ep, gr%mesh%np_part)
      else
        call get_electric_field_state(st%rs_state, dtmp, st%ep, gr%mesh%np_part)
      end if
      do idim = 1, st%d%dim
        write(fname, '(2a)') 'e_field-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr%mesh, dtmp(:,idim), fn_unit, ierr, geo = geo)
      end do
      if (hm%plane_waves) then
        call get_electric_field_state(st%rs_state_plane_waves, dtmp, st%ep, gr%mesh%np_part)
        do idim = 1, st%d%dim
          write(fname, '(2a)') 'free_e_field-', index2axis(idim)
          call dio_function_output(outp%how, dir, fname, gr%mesh, dtmp(:,idim), fn_unit, ierr, geo = geo)
        end do
      end if
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__MAGNETIC_FIELD) /= 0) then
      ...
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np_part,1:st%d%dim))
      call get_magnetic_field_state(st%rs_state, st%rs_sign, dtmp, st%mu, &
                                            gr%mesh%np_part)
      do idim = 1, st%d%dim
      ...
      if (hm%plane_waves) then
        call get_magnetic_field_state(st%rs_state_plane_waves, st%rs_sign, dtmp, st%mu, &
                                              gr%mesh%np_part)
        do idim = 1, st%d%dim
          write(fname, '(2a)') 'free_e_field-', index2axis(idim)
          call dio_function_output(outp%how, dir, fname, gr%mesh, dtmp(:,idim), fn_unit, ierr, geo = geo)
  ...
  subroutine output_energy_density_mxll(hm, gr, geo, dir, outp)   !< have to set unit output correctly
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__ENERGY_DENSITY) /= 0) then
      call dio_function_output(outp%how, dir, "maxwell_energy_density", gr%mesh, & 
                               hm%energy_density(:), units_out%energy/units_out%length**3, ierr, geo = geo)
      if (hm%plane_waves) then
      call dio_function_output(outp%how, dir, "free_maxwell_energy_density", gr%mesh, & 
                               hm%energy_density_plane_waves(:), units_out%energy/units_out%length**3, ierr, geo = geo)
      end if
    end if
  ...
  subroutine output_poynting_vector(st, gr, hm, geo, dir, outp)
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__POYNTING_VECTOR) /= 0) then
      ...
      if (hm%plane_waves) then
        call get_poynting_vector_plane_waves(gr, st, st%rs_sign, dtmp)
        do idim = 1, st%d%dim
          write(fname, '(2a)') 'free_poynting_vector-', index2axis(idim)
          call dio_function_output(outp%how, dir, fname, gr%mesh, dtmp(:,idim), fn_unit, ierr, geo = geo)
        end do
      end if
      SAFE_DEALLOCATE_A(dtmp)
  ...
  subroutine output_transverse_rs_state(st, gr, geo, dir, outp)    !< have to set unit output correctly
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__TRANS_ELECTRIC_FIELD) /= 0) then
      fn_unit = unit_one
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np,1:st%d%dim))
      call get_electric_field_state(st%rs_state_trans(1:gr%mesh%np,1:st%d%dim), &
                                            dtmp(1:gr%mesh%np,1:st%d%dim), &
                                            st%ep(1:gr%mesh%np), gr%mesh%np)
      do idim=1, st%d%dim
  ...
  subroutine output_longitudinal_rs_state(st, gr, geo, dir, outp)    !< have to set unit output correctly
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__LONG_ELECTRIC_FIELD) /= 0) then
      ...
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np,1:st%d%dim))
      ztmp(1:gr%mesh%np,:) = st%rs_state_long(1:gr%mesh%np,:) 
      call get_electric_field_state(ztmp, dtmp, st%ep(1:gr%mesh%np), gr%mesh%np)
      do idim=1, st%d%dim
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__LONG_MAGNETIC_FIELD) /= 0) then
      ...
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np,1:st%d%dim))
      ztmp(1:gr%mesh%np,:) = st%rs_state_long(1:gr%mesh%np,:)
      call get_magnetic_field_state(ztmp, st%rs_sign, dtmp, st%mu(1:gr%mesh%np), &
                                            gr%mesh%np)
      do idim=1, st%d%dim
  ...
  subroutine output_divergence_rs_state(st, gr, geo, dir, outp)    !< have to set unit output correctly
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__DIV_ELECTRIC_FIELD) /= 0) then
      fn_unit = unit_one
      SAFE_ALLOCATE(ztmp(1:gr%mesh%np_part,1))
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np_part,1))
      call get_divergence_field(gr, st%rs_state, ztmp(:,1), .false.)
      call get_magnetic_field_state(ztmp, st%rs_sign, dtmp, &
                                            st%mu(1:gr%mesh%np_part), gr%mesh%np_part)
      call dio_function_output(outp%how, dir, "e_field_div", gr%mesh, dtmp(:,1), fn_unit, ierr, geo=geo)
      SAFE_DEALLOCATE_A(ztmp)
      SAFE_DEALLOCATE_A(dtmp)
    end if
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__DIV_MAGNETIC_FIELD) /= 0) then
      fn_unit = unit_one
      SAFE_ALLOCATE(ztmp(1:gr%mesh%np,1))
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np,1))
      call get_divergence_field(gr, st%rs_state, ztmp(:,1), .false.)
      call get_magnetic_field_state(ztmp, st%rs_sign, dtmp, st%mu(1:gr%mesh%np), &
                                            gr%mesh%np)
      call dio_function_output(outp%how, dir, "b_field_div", gr%mesh, dtmp(:,1), fn_unit, ierr, geo=geo)
      SAFE_DEALLOCATE_A(ztmp)
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_vector_potential(st, gr, hm, geo, dir, outp)     !< have to set unit output correctly
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__VECTOR_POTENTIAL) /= 0) then
      fn_unit = unit_one
      SAFE_ALLOCATE(dtmp(1:gr%mesh%np))
       do idim = 1, gr%sb%dim
         write(fname, '(2a)') 'vector_potential-', index2axis(idim)
         dtmp(1:gr%mesh%np) = hm%vector_potential(1:gr%mesh%np,idim)
         call dio_function_output(outp%how, dir, fname, gr%mesh, dtmp, fn_unit, ierr, geo = geo)
       end do
      SAFE_DEALLOCATE_A(dtmp)
    end if
  ...
  subroutine output_coupling_potentials(hm, gr, geo, dir, outp)    !< have to set unit output correctly
    ...
    PUSH_SUB(output_coupling_potentials)
    ...
    if (iand(hm%ep%mx_ma_coupling_order, OPTION__MAXWELLCOUPLINGORDER__ELECTRIC_DIPOLE_COUPLING) /= 0) then
      if (iand(outp%what, OPTION__MAXWELLOUTPUT__ELECTRIC_DIPOLE_POTENTIAL) /= 0) then
        SAFE_ALLOCATE(dtmp(1:gr%mesh%np))
        fn_unit = unit_one
        dtmp(1:gr%mesh%np) = hm%ep%mx_ma_coupling_e_dipole_pot(1:gr%mesh%np)
        call dio_function_output(outp%how, dir, "electric_dipole_potential", gr%mesh, dtmp, fn_unit, ierr, geo = geo)
        SAFE_DEALLOCATE_A(dtmp)
      end if
    end if
    ...
    if (iand(hm%ep%mx_ma_coupling_order, OPTION__MAXWELLCOUPLINGORDER__ELECTRIC_QUADRUPOLE_COUPLING) /= 0) then
      if(iand(outp%what, OPTION__MAXWELLOUTPUT__ELECTRIC_QUADRUPOLE_POTENTIAL) /= 0) then
        SAFE_ALLOCATE(dtmp(1:gr%mesh%np))
        fn_unit = unit_one
        dtmp(1:gr%mesh%np) = hm%ep%mx_ma_coupling_e_quadrupole_pot(1:gr%mesh%np)
        call dio_function_output(outp%how, dir, "electric_quadrupole_potential", gr%mesh, dtmp, fn_unit, ierr, geo = geo)
        SAFE_DEALLOCATE_A(dtmp)
      end if
    end if

    POP_SUB(output_coupling_potentials)
  ...
  subroutine output_current_density(st_mxll, gr_mxll, hm_mxll, st_elec, gr_elec, hm_elec, geo, dir, time, outp)
    ...
    type(grid_t),              intent(inout) :: gr_mxll
    type(hamiltonian_mxll_t),       intent(in)    :: hm_mxll
    type(states_t),            intent(inout) :: st_elec
    ...
    PUSH_SUB(output_current_density)

    ! Maxwell current density
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__CURRENT) /= 0) then
      fn_unit = (unit_one/units_out%time)/(units_out%length**2)      !< test both if its the same
      SAFE_ALLOCATE(ztmp_1(1:gr_mxll%mesh%np_part,1:st_mxll%d%dim))
      SAFE_ALLOCATE(dtmp_1(1:gr_mxll%mesh%np_part,1:st_mxll%d%dim))
      ztmp_1 = M_z0
      dtmp_1 = M_ZERO
      if (hm_mxll%operator == OPTION__MAXWELLHAMILTONIANOPERATOR__FARADAY_AMPERE_GAUSS) then
        SAFE_ALLOCATE(ztmp_2(1:gr_mxll%mesh%np_part))
        ztmp_2 = m_Z0
      end if
      call get_rs_density(st_mxll, gr_mxll, hm_mxll, st_elec, gr_elec, hm_elec, geo, hm_elec%mx_ma_coupling_points, &
      ...
      if (hm_mxll%current_density_ext_flag) then
        SAFE_ALLOCATE(ztmp_3(1:gr_mxll%mesh%np_part,1:st_mxll%d%dim))
        call get_rs_density_ext(st_mxll, gr_mxll%mesh, time, ztmp_3)
        ztmp_1 = ztmp_1 + ztmp_3
        SAFE_DEALLOCATE_A(ztmp_3)
      end if
      call get_current_state(ztmp_1, dtmp_1, st_mxll%ep, gr_mxll%mesh%np_part)
      ! Check sign!
      dtmp_1 = - dtmp_1
      do idim = 1, st_mxll%d%dim
        write(fname, '(2a)') 'maxwell_current-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr_mxll%mesh, dtmp_1(:,idim), fn_unit, ierr, geo = geo)
      end do
      call get_rs_density(st_mxll, gr_mxll, hm_mxll, st_elec, gr_elec, hm, geo, hm%mx_ma_coupling_points, &
                                  hm%mx_ma_coupling_points_number, ztmp_1, ztmp_2, time, hm%current_prop_test)
      call helmholtz_decomposition_trans_field(hm_mxll%poisson_solver, gr_mxll, hm_mxll, hm, st_mxll, ztmp_1)
      call get_current_state(ztmp_1, dtmp_1, st_mxll%ep, gr_mxll%mesh%np_part)
      ! Check sign!
      dtmp_1 = - dtmp_1
      do idim = 1, st_mxll%d%dim
        write(fname, '(2a)') 'maxwell_current_trans-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr_mxll%mesh, dtmp_1(:,idim), fn_unit, ierr, geo = geo)
      end do
      call get_rs_density(st_mxll, gr_mxll, hm_mxll, st_elec, gr_elec, hm, geo, hm%mx_ma_coupling_points, &
                                  hm%mx_ma_coupling_points_number, ztmp_1, ztmp_2, time, hm%current_prop_test)
      call helmholtz_decomposition_long_field(hm_mxll%poisson_solver, gr_mxll, ztmp_1)
      call get_current_state(ztmp_1, dtmp_1, st_mxll%ep, gr_mxll%mesh%np_part)
      ! Check sign!
      dtmp_1 = - dtmp_1
      do idim = 1, st_mxll%d%dim
        write(fname, '(2a)') 'maxwell_current_long-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr_mxll%mesh, dtmp_1(:,idim), fn_unit, ierr, geo = geo)
      end do
    ...
    POP_SUB(output_current_density)
  ...
  subroutine output_external_current_density(st, gr, hm, geo, dir, time, outp)
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__EXTERNAL_CURRENT) /= 0) then
      if (hm%current_density_ext_flag) then
        ...
        call get_current_state(ztmp, dtmp, st%ep, gr%mesh%np_part)
        dtmp = - dtmp
        do idim = 1, st%d%dim
  ...
  !----------------------------------------------------------
  subroutine output_charge_density_mxll(st, gr, hm, geo, dir, outp, st, gr, hm)    !< have to set unit output correctly
    type(states_mxll_t),                intent(in)    :: st
    type(grid_t),                  intent(in)    :: gr
    type(hamiltonian_mxll_t),           intent(in)    :: hm
    type(geometry_t),              intent(in)    :: geo
    ...
    type(states_mxll_t),      optional, intent(in)    :: st
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__CHARGE_DENSITY) /= 0) then
      ...
      call get_divergence_field(gr, st%rs_state, ztmp(:,1), .true.)
      call get_electric_field_state(ztmp, dtmp_1, st%ep, gr%mesh%np_part)
      call dio_function_output(outp%how, dir, "charge_density", gr%mesh, dtmp_1(:,1), &
    ...
    if (present(st) .and. present(gr) .and. present(hm)) then
      if(iand(outp%what, OPTION__MAXWELLOUTPUT__CHARGE_DENSITY_DIFF) /= 0) then
        ...
        call dio_function_output(outp%how, dir, "charge_density_diff", gr%mesh, dtmp_2(:,1), &
                                fn_unit, ierr, geo=geo)
        SAFE_DEALLOCATE_A(dtmp_1)
  ...
  subroutine output_medium_variables_electric(st, gr, hm, geo, dir, outp, st_elec, gr_elec, hm_elec)    !< have to set unit output correctly
    ...
    type(states_t),      optional, intent(in)    :: st_elec
    type(hamiltonian_t), optional, intent(in)    :: hm_elec
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_ELECTRIC) /= 0) then
      ...
      do is = 1, st_elec%d%nspin
        forall(ip=1:gr_elec%mesh%np, idim=1:gr_elec%sb%dim) polarization_ma_gr(ip,idim) = &
               polarization_ma_gr(ip,idim) - st_elec%rho(ip,is)*(gr_elec%mesh%x(ip,idim)-hm_elec%mx_ma_coupling_points(idim,1))
      end do
      polarization_mx_gr = M_ZERO
      call dma_mesh_to_mx_mesh(st, gr, st_elec, gr_elec, polarization_ma_gr, polarization_mx_gr, 3)
      forall(ip=1:gr%mesh%np, idim=1:st%d%dim) D_field(ip,idim) = E_field(ip,idim)*P_ep - polarization_mx_gr(ip,idim)
      do idim = 1, st%d%dim
        write(fname, '(2a)') 'pol_density-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr%mesh, polarization_mx_gr(:,idim), fn_unit, ierr, geo = geo)
        write(fname, '(2a)') 'd_field-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr%mesh, D_field(:,idim), fn_unit, ierr, geo = geo)
      end do
      forall(ip=1:gr%mesh%np) e_susceptibility(ip) = sqrt(sum(D_field(ip,:)**2))/(P_ep*sqrt(sum(E_field(ip,:)**2))) - M_ONE
      write(fname, '(1a)') 'electric_susceptibility'
      call dio_function_output(outp%how, dir, fname, gr%mesh, e_susceptibility(:), fn_unit, ierr, geo = geo)
  ...
  subroutine output_medium_variables_magnetic(st, gr, hm, geo, dir, outp, st_elec, gr_elec, hm_elec)    !< have to set unit output correctly
    ...
    type(output_t),                intent(in)    :: outp
    type(grid_t),        optional, intent(inout) :: gr_elec
    type(states_t),      optional, intent(inout) :: st_elec
    type(hamiltonian_t), optional, intent(in)    :: hm_elec
    ...
    if(iand(outp%what, OPTION__MAXWELLOUTPUT__MEDIUM_VARIABLES_MAGNETIC) /= 0) then
      ...
      if (.not. hm%ma_mx_coupling) then
        call get_magnetic_field_state(st%rs_state_trans+st%rs_state_long, st%rs_sign, &
                                              B_field, st%mu, gr%mesh%np)
      else
        call get_magnetic_field_state(st%rs_state, st%rs_sign, B_field, st%mu, &
                                              gr%mesh%np)
      end if
      !call zstates_angular_momentum(st, gr, angular_momentum)
      call current_init(current_calc)
      call current_calculate(current_calc, gr_elec%der, hm_elec, geo, st_elec, current)
      magnetization_ma_gr = M_ZERO
      do ip=1, gr_elec%mesh%np
        do is = 1, st_elec%d%nspin
          !forall(ip=1:gr%mesh%np, idim=1:gr%sb%dim) magnetization_ma_gr(ip,idim) = magnetization_ma_gr(ip,idim) - M_HALF*st%rho(ip,is)*angular_momentum(ip,is,idim)
          magnetization_ma_gr(ip,:) = magnetization_ma_gr(ip,:) + M_HALF*dcross_product(gr_elec%mesh%x(ip,:)-hm%mx_ma_coupling_points(:,1),current(ip,:,is))
        end do
      end do
      magnetization_mx_gr = M_ZERO
      call dma_mesh_to_mx_mesh(st, gr, st_elec, gr_elec, magnetization_ma_gr, magnetization_mx_gr, 3)
      forall(ip=1:gr%mesh%np, idim=1:st%d%dim) H_field(ip,idim) = B_field(ip,idim)/P_mu - magnetization_mx_gr(ip,idim)
      do idim = 1, st%d%dim
        write(fname, '(2a)') 'mag_density-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr%mesh, magnetization_mx_gr(:,idim), fn_unit, ierr, geo = geo)
        write(fname, '(2a)') 'h_field-', index2axis(idim)
        call dio_function_output(outp%how, dir, fname, gr%mesh, H_field(:,idim), fn_unit, ierr, geo = geo)
      end do
      forall(ip=1:gr%mesh%np) b_susceptibility(ip) = sqrt(sum(H_field(ip,:)**2))/(P_ep*sqrt(sum(B_field(ip,:)**2))) - M_ONE
      write(fname, '(1a)') 'magnetic_susceptibility'
      call dio_function_output(outp%how, dir, fname, gr%mesh, b_susceptibility(:), fn_unit, ierr, geo = geo)
