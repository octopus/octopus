...
contains
  ...
  subroutine v_ks_init(ks, gr, st, geo, mc)
    ...
    if(ks%theory_level /= INDEPENDENT_PARTICLES) ks%hartree_solver => psolver
  ...
  ! ---------------------------------------------------------
  subroutine v_ks_calc(ks, hm, st, geo, calc_eigenval, time, calc_energy, calc_current)
    type(v_ks_t),               intent(inout) :: ks
    ...
    call v_ks_calc_start(ks, hm, st, geo, time, calc_energy, calc_current_)
  ...
  subroutine v_ks_calc_start(ks, hm, st, geo, time, calc_energy, calc_current) 
  ...
  contains
    ...
    subroutine calculate_density()
      ...
      call states_total_density(st, ks%gr%mesh, ks%calc%density)
    ...
    subroutine add_adsic(hm)
      ...
      case (UNPOLARIZED, SPIN_POLARIZED)
        do ispin = 1, st%d%nspin
          ...
          call xc_get_vxc(ks%gr%der, ks%xc, &
      ...
      if(ks%calc%calc_energy) then
        ...
        do ispin = 1, hm%d%nspin
          ...
          ks%calc%energy%intnvxc = ks%calc%energy%intnvxc + &
            factor*dmf_dotp(ks%gr%mesh, st%rho(:, ispin), ks%calc%vxc(:, ispin))
        end do
      end if
  ...
  subroutine v_ks_hartree(ks, hm)
    ...
    if(ks%calc%calc_energy) then
      ...
      hm%energy%hartree = M_HALF*dmf_dotp(ks%gr%mesh, ks%calc%total_density, pot)
    end if
