  ...
  contains
    ...
    subroutine add_adsic(hm)
      ...
      case (UNPOLARIZED, SPIN_POLARIZED)
        do ispin = 1, st%d%nspin
          ...
          call xc_get_derivatives(ks%gr%fine%der, ks%xc, st, hm%psolver_fine, namespace, rho, st%d%ispin, &
            -minval(st%eigenval(st%nst,:)), qsp(ispin), vxc=vxc_sic)
    ...
    subroutine v_a_xc(hm)
      ...
      if(ks%calc%calc_energy) then
        if (family_is_mgga_with_exc(hm%xc)) then
          call xc_get_derivatives(ks%gr%fine%der, ks%xc, st, hm%psolver_fine, namespace, &
            ks%calc%density, st%d%ispin, -minval(st%eigenval(st%nst,:)), st%qtot, vxc=ks%calc%vxc, &
        ...
        if (family_is_mgga_with_exc(hm%xc)) then
          call xc_get_derivatives(ks%gr%fine%der, ks%xc, st, hm%psolver_fine, namespace, &
            ks%calc%density, st%d%ispin, -minval(st%eigenval(st%nst,:)), st%qtot, vxc=ks%calc%vxc, vtau = ks%calc%vtau)
        else
          call xc_get_derivatives(ks%gr%fine%der, ks%xc, st, hm%psolver_fine, namespace, &
            ks%calc%density, st%d%ispin, -minval(st%eigenval(st%nst,:)), st%qtot, vxc=ks%calc%vxc)
        end if
