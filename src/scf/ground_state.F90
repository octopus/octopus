  ...
  subroutine ground_state_run(sys, hm, fromScratch)
    ...
    if(.not. fromScratch) then
      ...
      call restart_init(restart_load, RESTART_GS, RESTART_TYPE_LOAD, sys%st%dom_st_kpt_mpi_grp, &
                        sys%mc, ierr, mesh=sys%gr%mesh, exact = (sys%ks%theory_level == RDMFT))
      if(ierr == 0) &
    ...
    call restart_init(restart_dump, RESTART_GS, RESTART_TYPE_DUMP, sys%st%dom_st_kpt_mpi_grp, sys%mc, ierr, mesh=sys%gr%mesh)
