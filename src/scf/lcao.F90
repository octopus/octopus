...
!#define LCAO_DEBUG
  ...
  subroutine lcao_init(this, gr, geo, st)
    ...
    !%End
    call parse_variable(dummy_parser, 'LCAOStart', mode_default, this%mode)
    ...
    !%End
    call parse_variable(dummy_parser, 'LCAOAlternative', .false., this%alternative)
    ...
    if(states_are_complex(st)) then
      call parse_variable(dummy_parser, 'LCAOComplexYlms', .false., this%complex_ylms)
    else
    ...
    !!%End
#ifdef LCAO_DEBUG
    call parse_variable(dummy_parser, 'LCAODebug', .false., this%debug)
...
! mysterious problems with optimization on PGI 12.4.0.
    ...
    if(.not. this%alternative) then
      ...
      !%End
      call parse_variable(dummy_parser, 'LCAOScaleFactor', CNST(1.0), this%orbital_scale_factor)
      ...
      !%End
      call parse_variable(dummy_parser, 'LCAOMaximumOrbitalRadius', CNST(20.0), max_orb_radius, unit = units_inp%length)
      ...
      !%End
      call parse_variable(dummy_parser, 'LCAODimension', 0, n)
  ...
  contains
    ...
    subroutine lcao2_init()
      ...
      !%
      !%End
      call parse_variable(dummy_parser, 'LCAOKeepOrbitals', .true., this%keep_orb)
      ...
      !%End
      call parse_variable(dummy_parser, 'LCAOExtraOrbitals', .false., this%derivative)
      ...
      !%End
      call parse_variable(dummy_parser, 'LCAODiagTol', CNST(1e-10), this%diag_tol)
  ...
  subroutine lcao_guess_density(this, parser, st, gr, sb, geo, qtot, nspin, spin_channels, rho)
    ...
    if (spin_channels == 1) then
      ...
      !%End
      call parse_variable(dummy_parser, 'GuessMagnetDensity', INITRHO_FERROMAGNETIC, gmd_opt)
