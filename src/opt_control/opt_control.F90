...
contains
  ...
  contains
    ...
    subroutine scheme_zbr98()
      ...
      j1 = target_j1(oct_target, sys%gr, qcpsi, sys%geo, hm%ep)
      stop_loop = iteration_manager(j1, par, par_prev, iterator)
    ...
    subroutine scheme_cg()
      ...
      f = - target_j1(oct_target, sys%gr, qcpsi, sys%geo, hm%ep) - controlfunction_j2(par)
      call opt_control_state_end(qcpsi)
  ...
  subroutine f_wg05(sys, hm, td, qcpsi, par, prop_psi, prop_chi, j1)
    ...
    call target_chi(oct_target, sys%gr, qcpsi, qcchi, sys%geo, hm%ep)
