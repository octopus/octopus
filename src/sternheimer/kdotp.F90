...
contains
  ...
  subroutine kdotp_lr_run(sys, hm, fromScratch)
    ...
    call restart_init(restart_dump, RESTART_KDOTP, RESTART_TYPE_DUMP, sys%st%dom_st_kpt_mpi_grp, sys%mc, ierr, mesh=sys%gr%mesh)
    ! no problem if this fails
    call restart_init(restart_load, RESTART_KDOTP, RESTART_TYPE_LOAD, sys%st%dom_st_kpt_mpi_grp, sys%mc, ierr, mesh=sys%gr%mesh)
