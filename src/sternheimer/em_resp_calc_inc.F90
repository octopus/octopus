...
subroutine X(lr_calc_magnetization_periodic)(sys, lr_k, magn)
  ...
  do ik = sys%st%d%kpt%start, sys%st%d%kpt%end
    ...
    do ist = 1, sys%st%nst
      if (abs(sys%st%occ(ist, ik)) .gt. M_EPSILON) then
        do idir = 1, ndir
          call X(hamiltonian_elec_apply)(sys%hm, sys%gr%der, lr_k(idir)%X(dl_psi)(:, :, ist, ik), &
            Hdl_psi(:, :, idir), ist, ik)
        end do
...
subroutine X(lr_calc_susceptibility_periodic)(sys, nsigma, lr_k, lr_b, &
  ...
  do ik = sys%st%d%kpt%start, sys%st%d%kpt%end
    ...
    do ist = 1, sys%st%nst
      if(abs(sys%st%occ(ist, ik)) .gt. M_EPSILON) then
        do idir = 1, ndir
          call X(hamiltonian_elec_apply)(sys%hm, sys%gr%der, lr_b(idir)%X(dl_psi)(:,:,ist,ik),&
            Hdl_b(:,:,idir), ist, ik)
          call X(hamiltonian_elec_apply)(sys%hm, sys%gr%der, lr_k(idir)%X(dl_psi)(:,:,ist,ik),&
        ...
        do idir1 = 1, ndir
          do idir2 = 1, ndir
            call X(hamiltonian_elec_apply)(sys%hm, sys%gr%der, lr_kb(idir1, idir2)%X(dl_psi)(:,:, ist, ik),&
              Hdl_kb(:,:, idir1, idir2), ist, ik)
            call X(hamiltonian_elec_apply)(sys%hm, sys%gr%der, &
