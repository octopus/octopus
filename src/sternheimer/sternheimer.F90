...
contains
  ...
  subroutine sternheimer_init(this, sys, hm, wfs_are_cplx, &
    ...
    call parse_variable(dummy_parser, 'Preorthogonalization', default_preorthog, this%preorthogonalization) 
    ...
    if(present(set_ham_var)) then
      ...
      call parse_variable(dummy_parser, 'HamiltonianVariation', 3, ham_var)
    else
      ham_var = 0
    end if
