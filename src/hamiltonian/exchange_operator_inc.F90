...
subroutine X(exchange_operator_single)(this, der, st_d, ist, ik, psi, hpsi, exxcoef)
  ...
  call X(exchange_operator_apply)(this, der, st_d, ik, psib, hpsib, exxcoef)
...
subroutine X(exchange_operator_apply)(this, der, st_d, ik, psib, hpsib, exxcoef)
  type(exchange_operator_t), intent(inout) :: this
  type(derivatives_t),       intent(in)    :: der
  type(states_dim_t),        intent(in)    :: st_d
  integer,                   intent(in)    :: ik
  ...
  if(present(exxcoef)) then
    ...
    exx_coef = max(this%exx_coef,this%cam_beta)
  end if
  ...
  do ibatch = 1, psib%nst
    ...
    do ik2 = 1, st_d%nik
      ...
      ikpoint2 = states_dim_get_kpoint_index(st_d, ik2)
...
subroutine X(exchange_operator_hartree_apply) (this, der, st_d, ik, exx_coef, psib, hpsib)
  type(exchange_operator_t), intent(inout) :: this
  type(derivatives_t),       intent(in)    :: der
  type(states_dim_t),        intent(in)    :: st_d
  integer,                   intent(in)    :: ik
  ...
  do ibatch = 1, psib%nst
    ...
    do ik2 = 1, st_d%nik
      ...
      call states_get_state(this%st, der%mesh, ist, ik2, psi2)
...
! scdm_EXX
! ---------------------------------------------------------
subroutine X(exchange_operator_scdm_apply) (this, der, st_d, psib, hpsib, ik, exx_coef, hartree)
  type(exchange_operator_t), intent(inout) :: this
  type(derivatives_t),       intent(in)    :: der
  type(states_dim_t),        intent(in)    :: st_d
  type(batch_t),             intent(inout) :: psib
  ...
  call scdm_init(this%st, der, psolver%cube, this%scdm)
  ! make sure scdm is localized
  call X(scdm_localize)(this%st, der%mesh, this%scdm)
...
! ---------------------------------------------------------
subroutine X(exchange_operator_rdmft_apply) (this, der, st_d, ik, psib, hpsib)
  ...
  PUSH_SUB(X(exchange_operator_rdmft_apply))
  ...
  SAFE_ALLOCATE(rho(1:der%mesh%np))
  SAFE_ALLOCATE(pot(1:der%mesh%np))
  SAFE_ALLOCATE(psi1(1:der%mesh%np,1:st_d%dim))
  ...
  do ibatch = 1, psib%nst
    ...
    do jst = 1,this%st%nst
      pot = R_TOTYPE(M_ZERO)
      psi1 = M_ZERO
      rho = R_TOTYPE(M_ZERO)
      call states_get_state(this%st, der%mesh, jst, 1 , psi1)
      forall(ip = 1:der%mesh%np)
        rho(ip) =  R_CONJ(psi(ip, 1))*psi1(ip, 1)
      end forall
      call X(poisson_solve)(psolver, pot, rho)
      forall(ip = 1:der%mesh%np)
        pot(ip) = pot(ip)*R_CONJ(psi1(ip, 1))*sqrt(this%st%occ(jst, 1)) !Mueller functional
      end forall
      forall(ip = 1:der%mesh%np)
  ...
  POP_SUB(X(exchange_operator_rdmft_apply))
end subroutine X(exchange_operator_rdmft_apply)
