...
subroutine X(hamiltonian_apply_batch) (hm, der, psib, hpsib, ik, terms, set_bc, set_phase)
 ...
 if(hm%F%floquet_apply) then
     call X(apply_floquet_hamiltonian)(hm, der, psib, hpsib, ik, terms, set_bc, set_phase)
  else
     call X(hamiltonian_apply_batch2)(hm, der, psib, hpsib, ik, terms, set_bc, set_phase)
  end if
...
! ---------------------------------------------------------
subroutine X(hamiltonian_apply_batch2) (hm, der, psib, hpsib, ik, terms, set_bc, set_phase)
  type(hamiltonian_t),   intent(in)    :: hm
  type(derivatives_t),   intent(in)    :: der
  type(batch_t), target, intent(inout) :: psib
  type(batch_t), target, intent(inout) :: hpsib
...
! ---------------------------------------------------------
subroutine  X(apply_floquet_hamiltonian)(hm, der, psib, hpsib, ik, terms, set_bc, set_phase)
  type(hamiltonian_t),   intent(in)    :: hm
  type(derivatives_t),   intent(in)    :: der
  type(batch_t), target, intent(inout) :: psib
  type(batch_t), target, intent(inout) :: hpsib
  ...
  case (OPTION__FLOQUETBOSON__CLASSICAL_FLOQUET)
    ...
    do idx=hm%F%flat_idx%start, hm%F%flat_idx%end
      ...
      call X(hamiltonian_apply_batch2)(hm%td_hm(it), der, small_psib , small_hpsib, ik, &
                                       terms = terms, set_bc = set_bc, set_phase = set_phase)
