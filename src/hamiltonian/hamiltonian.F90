...
contains
  ...
  subroutine hamiltonian_init(hm, parser, gr, geo, st, theory_level, xc_family, family_is_mgga_with_exc)
    ...
    !% esoteric purposes.
    !%End
    call parse_variable(dummy_parser, 'ParticleMass', M_ONE, hm%mass)
    ...
    !%End
    call parse_variable(dummy_parser, 'RashbaSpinOrbitCoupling', M_ZERO, rashba_coupling, units_inp%energy*units_inp%length)
    ...
    call parse_variable(dummy_parser, 'CalculateSelfInducedMagneticField', .false., hm%self_induced_magnetic)
    ...
    !%End
    call parse_variable(dummy_parser, 'DFTULevel', DFT_U_NONE, hm%lda_u_level)
    ...
    !%End
    call parse_variable(dummy_parser, 'HamiltonianApplyPacked', .true., hm%apply_packed)
    ...
    !%End
    call parse_variable(dummy_parser, 'scdm_EXX', .false., hm%scdm_EXX)
    if(hm%scdm_EXX) then
    ...
    !% included.
    !%End
    call parse_variable(dummy_parser, 'TimeZero', .false., hm%time_zero)
  ...
  ! ---------------------------------------------------------
  subroutine hamiltonian_update(this, mesh, boundaries, time)
  ...
  subroutine hamiltonian_update2(this, mesh, time, mu)
    ...
    do itime = 1, 2
      ...
      do ilaser = 1, this%ep%no_lasers
        ...
        case(E_FIELD_MAGNETIC)
          call hamiltonian_base_allocate(this%hm_base, mesh, FIELD_VECTOR_POTENTIAL + FIELD_UNIFORM_MAGNETIC_FIELD, .false.)
          ! get the vector potential
          SAFE_ALLOCATE(vp(1:mesh%np, 1:mesh%sb%dim))
          vp(1:mesh%np, 1:mesh%sb%dim) = M_ZERO
          call laser_vector_potential(this%ep%lasers(ilaser), mesh, vp, time_)
