...
subroutine xc_get_nc_vxc(der, xcs, st, kpoints, namespace, rho, vxc, ex, ec, vtau, ex_density, ec_density)
  ...
  call profiling_in(prof, "X(XC_NC_LOCAL)")
...
contains
  ...
  subroutine nc_lda_init()
    ...
    do ii = 1, der%mesh%np
      ...
      dens(ii, 3) = max(rho(ii, 3), M_ZERO)
      dens(ii, 4) = max(rho(ii, 4), M_ZERO)
    end do
  ...
  subroutine nc_br_vxc_exc(l_dens, l_sigma, l_ldens, l_tau, l_dedd, l_vsigma, l_dedldens, l_dedtau, l_zk)
    ...
    FLOAT :: l_ontop(2), l_curv(2), xx(2)
    FLOAT :: cnst(2), Uup, Udn, Pup, Pdn, dUupdx, dUdndx
    FLOAT :: dtop_u_dn(3), dtop_d_dn(3), dcurv_u_dn(3), dcurv_d_dn(3)
    CMPLX :: tmp
    FLOAT, parameter :: tol_Q = CNST(1e-15)
    FLOAT, parameter :: tol_x = CNST(1e-7)
    FLOAT, parameter :: gamma = CNST(0.8)
    
    !We first compute the value of the on-top effective exchange hole
    l_ontop(1) = l_dens(1)
    l_ontop(2) = l_dens(2)
    if((l_dens(3)**2 + l_dens(4)**2) > tol_Q) then
      l_ontop(1) = l_ontop(1) + (l_dens(3)**2 + l_dens(4)**2)/(l_dens(1)+M_EPSILON)
      l_ontop(2) = l_ontop(2) + (l_dens(3)**2 + l_dens(4)**2)/(l_dens(2)+M_EPSILON)
    end if

    !We now add the rest of the curvature and we normalize it properly
    l_curv(1:2) = l_ldens(1:2) - M_TWO*gamma*(l_tau(1:2) - M_FOURTH*l_sigma(1:2)/(l_dens(1:2)+M_EPSILON))
    !Off diagonal contribution
    tmp = l_dens(3)*(l_ldens(3)-M_TWO*gamma*l_tau(3)) + l_dens(4)*(l_ldens(4)-M_TWO*gamma*l_tau(4))
    l_curv(1:2) = (l_curv(1:2) + tmp / (l_dens(1:2)+M_EPSILON)) / CNST(6.0)

    if(abs(l_curv(1)) < tol_Q) l_curv(1) = sign(tol_Q, l_curv(1))    
    if(abs(l_curv(2)) < tol_Q) l_curv(2) = sign(tol_Q, l_curv(2))    

    !We get the value of x for up and down components
    xx(1) = nc_br_get_x(l_ontop(1), l_curv(1))
    xx(2) = nc_br_get_x(l_ontop(2), l_curv(2))
    if(any(xx < M_ZERO)) then
      message(1) = "Newton-Raphson produced a negative x value"
      call messages_fatal(1)
    end if

    cnst(1:2) = M_TWO * (M_PI*l_ontop(1:2))**M_THIRD

    !We construct the enengy density 
    if (abs(xx(1)) > tol_x) then
      Uup = -( M_ONE - exp(-xx(1))*(M_ONE + M_HALF*xx(1)) ) & 
               / xx(1) * exp(xx(1)*M_THIRD) * cnst(1)
    else
      !Taylor expansion at x=0
      Uup = -cnst(1) * (M_HALF + xx(1)/CNST(6.0) - xx(1)**2/CNST(18.0))
    end if

    if(abs(xx(2)) > tol_x) then
      Udn = -( M_ONE - exp(-xx(2))*(M_ONE + M_HALF*xx(2)) ) &
                / xx(2) * exp(xx(2)*M_THIRD) * cnst(2)
    else
      !Taylor expansion at x=0
      Udn = -cnst(2) * (M_HALF + xx(2)/CNST(6.0) - xx(2)**2/CNST(18.0))
    end if
    ...
    if(abs(xx(1)) > tol_x) then
      dUupdx = (xx(1)**2 + M_TWO*xx(1) + exp(xx(1))*(xx(1)-M_THREE) + M_THREE) &
               * exp(-xx(1)*M_TWOTHIRD) / (M_THREE * xx(1)**2)
    else
      !Taylor expansion at x=0
      dUupdx = M_ONE/CNST(6.0) - xx(1)/CNST(9.0)
    end if
    dUupdx = dUupdx * cnst(1)

    if(abs(xx(2)) > tol_x) then
      dUdndx = (xx(2)**2 + M_TWO*xx(2) + exp(xx(2))*(xx(2)-M_THREE) + M_THREE) &
              * exp(-xx(2)*M_TWOTHIRD) / (M_THREE * xx(2)**2) 
    else
      !Taylor expansion at x=0
      dUdndx = M_ONE/CNST(6.0) - xx(2)/CNST(9.0)
    end if
    dUdndx = dUdndx * cnst(2)

    Pup = dUupdx * (xx(1)-M_TWO)*xx(1)/(xx(1)**2-M_TWO*xx(1)+M_THREE) &
            * M_THREE * M_HALF * l_dens(1) / (l_ontop(1)+M_EPSILON)
    Pdn = dUdndx * (xx(2)-M_TWO)*xx(2)/(xx(2)**2-M_TWO*xx(2)+M_THREE) &
            * M_THREE * M_HALF * l_dens(2) / (l_ontop(2)+M_EPSILON)

    ! Derivative of the on-top density wrt the density
    if((l_dens(3)**2 + l_dens(4)**2) > tol_Q) then
      dtop_u_dn(1) = M_ONE - (l_dens(3)**2 + l_dens(4)**2)/(l_dens(1)**2+M_EPSILON)
      dtop_d_dn(1) = M_ONE - (l_dens(3)**2 + l_dens(4)**2)/(l_dens(2)**2+M_EPSILON)
      dtop_u_dn(2) = l_dens(3)/(l_dens(1)+M_EPSILON)
      dtop_u_dn(3) =-l_dens(4)/(l_dens(1)+M_EPSILON)
      dtop_d_dn(2) = l_dens(3)/(l_dens(2)+M_EPSILON)
      dtop_d_dn(3) =-l_dens(4)/(l_dens(2)+M_EPSILON)
    else
      dtop_u_dn(1) = M_ONE
      dtop_d_dn(1) = M_ONE
      dtop_u_dn(2:3) = M_ZERO
      dtop_d_dn(2:3) = M_ZERO
    end if

    ! Derivative of the curvature up wrt the density
    dcurv_u_dn(1) =-M_ONE/(l_dens(1)**2+M_EPSILON) * ( M_HALF*gamma*l_sigma(1) &
               +  (l_dens(3)*(l_ldens(3) - gamma*M_TWO*l_tau(3))) &
               +  (l_dens(4)*(l_ldens(4) - gamma*M_TWO*l_tau(4))))
    dcurv_u_dn(2) = M_HALF/(l_dens(1)+M_EPSILON) * ( l_ldens(3) - M_TWO*gamma*l_tau(3) )
    dcurv_u_dn(3) =-M_HALF/(l_dens(1)+M_EPSILON) * ( l_ldens(4) - M_TWO*gamma*l_tau(4) )
    ! Derivative of the curvature down wrt the density
    dcurv_d_dn(1) =-M_ONE/(l_dens(2)**2+M_EPSILON) * ( M_HALF*gamma*l_sigma(2) &
               +  (l_dens(3)*(l_ldens(3) - gamma*M_TWO*l_tau(3))) &
               -  (l_dens(4)*(l_ldens(4) - gamma*M_TWO*l_tau(4))))
    dcurv_d_dn(2) = M_HALF/(l_dens(2)+M_EPSILON) * ( l_ldens(3) - M_TWO*gamma*l_tau(3) )
    dcurv_d_dn(3) =-M_HALF/(l_dens(2)+M_EPSILON) * ( l_ldens(4) - M_TWO*gamma*l_tau(4) )

    dcurv_u_dn(1:3) = dcurv_u_dn(1:3) / CNST(6.0)
    dcurv_d_dn(1:3) = dcurv_d_dn(1:3) / CNST(6.0)
    
    ! Derivative of the energy wrt the density
    l_dedd(1) = (M_ONE+M_THIRD*l_dens(1)/(l_ontop(1)+M_EPSILON)*dtop_u_dn(1))*Uup &
                  + Pup*(M_FIVE*M_THIRD*dtop_u_dn(1) - l_ontop(1)/l_curv(1)*dcurv_u_dn(1))
    l_dedd(2) = (M_ONE+M_THIRD*l_dens(2)/(l_ontop(2)+M_EPSILON)*dtop_d_dn(1))*Udn &
                  + Pdn*(M_FIVE*M_THIRD*dtop_d_dn(1) - l_ontop(2)/l_curv(2)*dcurv_d_dn(1))
    if((l_dens(3)**2 + l_dens(4)**2) > tol_Q) then
      l_dedd(3) = M_THIRD*l_dens(3)* (Uup/(l_ontop(1)+M_EPSILON) + Udn/(l_ontop(2)+M_EPSILON))
      l_dedd(4) =-M_THIRD*l_dens(4)* (Uup/(l_ontop(1)+M_EPSILON) + Udn/(l_ontop(2)+M_EPSILON)) 
    else
      l_dedd(3:4) = M_ZERO
    end if
    l_dedd(3) = l_dedd(3) + Pup*( M_FIVE*M_THIRD*dtop_u_dn(2) - l_ontop(1)/l_curv(1) * dcurv_u_dn(2)) &
                          + Pdn*( M_FIVE*M_THIRD*dtop_d_dn(2) - l_ontop(2)/l_curv(2) * dcurv_d_dn(2))
    l_dedd(4) = l_dedd(4) + Pup*( M_FIVE*M_THIRD*dtop_u_dn(3) - l_ontop(1)/l_curv(1) * dcurv_u_dn(3)) &
                           +Pdn*( M_FIVE*M_THIRD*dtop_d_dn(3) - l_ontop(2)/l_curv(2) * dcurv_d_dn(3))
    l_dedd(1:4) = l_dedd(1:4)*M_HALF

    !Here we recompute Pup and Pdn to avoid diving by l_curv, which can be very small
    Pup = dUupdx * xx(1)**2/(xx(1)**2-M_TWO*xx(1)+M_THREE) * exp(-xx(1)*M_TWOTHIRD) &
              * M_THREE * M_HALF * l_dens(1) / (l_ontop(1)+M_EPSILON) * M_THREE / (cnst(1)**2+M_EPSILON)
    Pdn = dUdndx * xx(2)**2/(xx(2)**2-M_TWO*xx(2)+M_THREE) * exp(-xx(2)*M_TWOTHIRD) &
              * M_THREE * M_HALF * l_dens(2) / (l_ontop(2)+M_EPSILON) * M_THREE / (cnst(2)**2+M_EPSILON)

    ! Derivative of the energy wrt sigma

    l_vsigma(1) =-M_HALF * Pup / (l_dens(1) + M_EPSILON)
    l_vsigma(2) =-M_HALF * Pdn / (l_dens(2) + M_EPSILON)
    l_vsigma(3:4) = M_ZERO
    ...
    l_dedldens(1) =-Pup 
    l_dedldens(2) =-Pdn
    l_dedldens(3) =-M_HALF * l_dens(3) * (Pup/(l_dens(1)+M_EPSILON) + Pdn/(l_dens(2)+M_EPSILON))
    l_dedldens(4) = M_HALF * l_dens(4) * (Pup/(l_dens(1)+M_EPSILON) + Pdn/(l_dens(2)+M_EPSILON))
    l_dedldens(1:4) = l_dedldens(1:4) / CNST(12.0)

    ! Derivative of the energy wrt the kinetic energy
    l_dedtau(1) = M_TWO * Pup
    l_dedtau(2) = M_TWO * Pdn
    l_dedtau(3) = l_dens(3) *(Pup/(l_dens(1)+M_EPSILON) + Pdn/(l_dens(2)+M_EPSILON))
    l_dedtau(4) =-l_dens(4) *(Pup/(l_dens(1)+M_EPSILON) + Pdn/(l_dens(2)+M_EPSILON))
    l_dedtau(1:4) = l_dedtau(1:4) * gamma / CNST(12.0)
  ...
  function nc_br_get_x(ldens, lcurv) result(br_x)
    ...
    FLOAT :: rhs
    FLOAT, parameter :: tol = CNST(1e-12)

    rhs = M_TWOTHIRD * (M_PI * ldens) ** M_TWOTHIRD * ldens / lcurv
  ...
  FLOAT function nc_br_rtsafe(rhs, tol) result(br_x)
    ...
    emx = exp(-M_TWO/M_THREE*br_x)
    ff = br_x * emx / rhs - (br_x-M_TWO)
    dff = emx * ( M_ONE - M_TWO/M_THREE * br_x) / rhs - M_ONE
    ...
    do it = 1, maxit
      ...
      ff = br_x * emx / rhs - (br_x-M_TWO)
      dff = emx * ( M_ONE - M_TWO/M_THREE * br_x) / rhs - M_ONE
      if(ff < 0) then ! Maintain the bracket on the root
        xl = br_x
      else
        xh = br_x
      end if
