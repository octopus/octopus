...
module scdm_oct_m
  ...
  type scdm_t
    ...
    FLOAT,   allocatable, public :: center(:,:) !< coordinates of centers of states (in same units as mesh%x)
    ...
    integer, allocatable, public :: box(:,:,:,:)  !< indices of global points that are contained in the local box for each state
    
    integer                      :: full_cube_n(3) !< dimension of cube of fullsimulation cell
    
    FLOAT,   allocatable, public :: dpsi(:,:)   !< scdm states in their local box
    CMPLX,   allocatable, public :: zpsi(:,:)   ! ^
