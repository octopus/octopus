   ...
   if(mpi_grp_is_root(mpi_world)) then
     ...
     do ios = 1, this%norbsets
       do icopies = 1, this%orbsets(ios)%nneighbors
         if(this%orbsets(ios)%ndim == 1) then
           if(this%orbsets(ios)%nn /= 0 ) then
             write(iunit,'(i4,a10, 2x, i1, a1, i2, f15.6)') ios, trim(species_label(this%orbsets(ios)%spec)), &
                                             this%orbsets(ios)%nn, l_notation(this%orbsets(ios)%ll), icopies, &
                                             units_from_atomic(units_out%energy, this%orbsets(ios)%V_IJ(icopies,0))
           else
             write(iunit,'(i4,a10, 3x, a1, i2, f15.6)') ios, trim(species_label(this%orbsets(ios)%spec)), &
                                             l_notation(this%orbsets(ios)%ll), icopies,                   &
                                             units_from_atomic(units_out%energy, this%orbsets(ios)%V_IJ(icopies,0))
           end if
        else
          if(this%orbsets(ios)%nn /= 0 ) then
