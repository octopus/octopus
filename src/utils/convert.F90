...
contains
  ...
  subroutine convert()
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertFilename', 'density', basename)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertHow', CONVERT_FORMAT, c_how)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertIterateFolder', .true., iterate_folder)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertFolder', folder_default, folder)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertStart', c_start_default, c_start)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertEnd', 1, c_end)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertStep', 1, c_step)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertSubtractFilename', 'density', ref_name)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertSubtract', .false., subtract_file)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertSubtractFolder', '.', ref_folder)
    call add_last_slash(folder)
  ...
  subroutine convert_transform(mesh, parser, geo, mc, basename, in_folder, c_start, c_end, c_step, outp, & 
    ...
    fdefault = M_ZERO
    call parse_variable(dummy_parser, 'TDTimeStep', fdefault, dt, unit = units_inp%time)
    if (dt <= M_ZERO) then
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertEnergyMin', M_ZERO, min_energy, units_inp%energy)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertReadSize', mesh%np, chunk_size)
    ...
    call parse_variable(dummy_parser, 'ConvertEnergyMax',fdefault, max_energy, units_inp%energy)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertFTMethod', 1, ft_method)
    ...
    call kick_init(kick, parser, 1, mesh%sb%dim, geo%periodic_dim)
  ...
  subroutine convert_operate(mesh, geo, mc, outp)
    ...
    !% <tt>convert</tt>. 
    !%End
    call parse_variable(dummy_parser, 'ConvertOutputFolder', "convert", out_folder)
    ...
    !%End
    call parse_variable(dummy_parser, 'ConvertOutputFilename', 'density', out_filename)
