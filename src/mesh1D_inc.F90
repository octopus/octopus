...
subroutine R_FUNC(mesh_to_cube) (m, f_mesh, f_cube)
  ...
  R_TYPE, intent(inout) :: f_cube(m%fft_n)
...
subroutine R_FUNC(cube_to_mesh) (m, f_cube, f_mesh)
  type(mesh_type), intent(IN) :: m
  R_TYPE, intent(IN)    :: f_cube(m%fft_n)
...
subroutine R_FUNC(mesh_derivatives) (m, f, lapl, grad)
  ...
  R_TYPE, intent(out), optional:: lapl(1:m%np), grad(1:m%np)
...
!!$    return
  ...
  subroutine rs_derivative()
    R_TYPE :: den1(1), den2(1)
    integer :: k, in, ind1(1), ind2(1), ix, iy, iz
    ...
    do k = 1, m%np
      ...
      ix = m%Lx(k)
      do in = 1, m%d%norder
        ind1(1) = m%Lxyz_inv(ix-in)
        ind2(1) = m%Lxyz_inv(ix+in)
        ...
        if(present(grad)) then
          grad(k) = grad(k) + m%d%dgidfj(-in)*den1(1) + m%d%dgidfj(in)*den2(1)
        end if
