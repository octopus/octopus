...
oct-broad - Broadens the linear-response spectra.
...
[oct-broad does not read standard input: all standard input
will be simply ignored. An input file named F<inp> must be present
in the running directory.
Also, oct-broad accepts no command line arguments, since there is
...
It post-processes the files F<linear/casida>,
F<linear/petersilka> and F<linear/eigenvalues>, that
contain the (WARNING: MISSING SHORT DESCRIPTION OF THE PROGRAM).
...
At the usual standard locations, you will also find and info file, F<octopus.info>.
